...
if ($_POST["Command"] == "sendproduction") {
    try {
        ...
        $sql = "UPDATE dag_item set flag='1'  where  refno='" . $_POST['refno'] . "' and  serialno='".$_POST['serialno']."'"; 
        $result = $conn->query($sql); 

        $sqllog = "insert into entry_log(refno, username, docname, trnType, stime, sdate) values ('" . trim($_POST['refno']) . "', '" . $_SESSION["CURRENT_USER"] . "', 'SEND COM TO PRO', 'Save', '" . date("Y-m-d H:i:s") . "', '" . date("Y-m-d") . "')";
