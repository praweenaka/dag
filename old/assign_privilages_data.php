...
if ($_GET["Command"] == "select_permission") {
    ...
    while ($row1 = mysqli_fetch_array($result1)) {
        ...
        if ($_GET["sam_user"] == "olduser") {
            $sql = "Select * from userpermission   where username='" . trim($_GET["user_name"]) . "' and docid=" . $row1["docid"] . "";
        } else {
            $sql = "Select * from userpermission   where username='" . trim($_GET["sam_user"]) . "' and docid=" . $row1["docid"] . "";
        }
...
if ($_POST["Command"] == "save_inv") {
    ...
    if ($_SESSION["CURRENT_USER"] == 'admin' or $_SESSION["CURRENT_USER"] == 'Admin' or $_SESSION["CURRENT_USER"] == 'buddhika1' or $_SESSION["CURRENT_USER"] == 'Buddhika1') {
        $pass = "";
        ...
        while ($row1 = mysqli_fetch_array($result1)) {
            ...
            $sql = "insert into userpermission(username, userpass, grp, docid, doc_view, doc_feed, doc_mod, price_edit, admin, dev, doc_print,doc_name) values  ('" . $_POST["user_name"] . "', '" . $pass . "', '" . $row1["grp"] . "', " . $row1["docid"] . ", " . $view . ", " . $feed . ", " . $mod . ", " . $price . ", " . $admin . ", " . $dev . ", " . $print . ",'" . $row1["docname"] . "')";
            //echo $sql;
            $result = mysqli_query($GLOBALS['dbinv'], $sql);

//            $sql_log = "insert into entry_log (refno, username, docid, docname, trnType, stime, sdate)  VALUES ('" . $_POST["user_name"] . "', '" . $_SESSION["CURRENT_USER"] . "', " . $_SESSION["CURRENT_DOC"] . " , '" . $row1["docid"] . "' ,'User Permition Save', '" . date("H:i:s") . "', '" . date("Y-m-d") . "')";
//            $result1 = mysqli_query($GLOBALS['dbinv'], $sql_log);
