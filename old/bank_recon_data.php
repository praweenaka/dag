...
if($_GET["Command"]=="display")
{
	...
	$mRow = 1;
	$txtDeb = 0;
	$ResponseXML .= "<debtable><![CDATA[<table width=\"300\" border=\"1\" cellspacing=\"0\">
         <tr>
	...
	$sql_opCh="Select * from op_cheq where (recdate='0'  OR recmonth='" . date("m", strtotime($_GET["dtpDate"])) . "')  and flag='DEB'  and code='" . trim($_GET["txtBankCode"]) . "' order by sdate";
	...
	while ($row_opCh = mysql_fetch_array($result_opCh)){
        ...
        $ResponseXML .=  "<tr>
          ...
          $ResponseXML .=  "<td>Last Year Balance</td>
		  	<td><input type=\"checkbox\" name=\"".$row_opCh["refno"]."\" id=\"".$row_opCh["refno"]."\" onclick=\"checkdb('".$row_opCh["refno"]."', '', 'DEB');\" /></td>
         
        </tr>";
		
		$sql="insert into tmpbankrecon_deb (ref_no, trn_date, chq_no, name, heading, amt, f, descrip, chk, id, user) values ('".$row_opCh["refno"]."', '".$row_opCh["sdate"]."', '".$row_opCh["chno"]."', '', '', ".$row_opCh["amount"].", '".$F."', 'Last Year Balance', '0', 0, '".$_SESSION['UserName']."')";
		$result=mysql_query($sql, $dbacc);
	}	
	...
	while ($row_strsql = mysql_fetch_array($result_strsql)){
	 ...
	 if (($row_strsql["l_flag2"] == "0") or (date("m", strtotime($row_strsql["l_date"])) == date("m", strtotime($_GET["dtpDate"]))) and (date("Y", strtotime($row_strsql["l_date"])) == date("Y", strtotime($_GET["dtpDate"])))) {
		...
		$ResponseXML .=  "<tr>
		  ...
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
          <td align=right>".number_format($row_strsql["l_amount"], 2, ".", ",")."</td>";
		  ...
		  if (($row_strsql["l_flag2"] == "1") or ($row_strsql["l_flag4"] == "CAS")) {
	       ...
	       if ($row_rspaymas = mysql_fetch_array($result_rspaymas)){
			...
			if ($row_strsql["l_flag2"] != "0") { 
				$ResponseXML .=  "<td><input type=\"checkbox\" name=\"".$row_strsql["l_refno"]."\" id=\"".$row_strsql["l_refno"]."\" onclick=\"checkdb('".$row_strsql["l_refno"]."', '".$row_strsql["id"]."', 'DEB');\" checked=\"checked\" /></td>";
				$l_flag2="1";
			} else {
				$ResponseXML .=  "<td><input type=\"checkbox\" name=\"".$row_strsql["l_refno"]."\" id=\"".$row_strsql["l_refno"]."\" onclick=\"checkdb('".$row_strsql["l_refno"]."', '".$row_strsql["id"]."', 'DEB');\" /></td>";
				$l_flag2="0";
			}	
   ...
   $ResponseXML .= "<cretable><![CDATA[<table width=\"300\" border=\"1\" cellspacing=\"0\">
         <tr>
          <td background=\"images/headingbg.gif\" height=\"25\">Ref No</td>
          <td background=\"images/headingbg.gif\">Date</td>
          <td background=\"images/headingbg.gif\">Cheque No</td>
   ...
   //////// CREDIT /////////////////
	...
	while ($row_opCh = mysql_fetch_array($result_opCh)){
		$ResponseXML .=  "<tr>
		  ...
		  <td><input type=\"checkbox\" name=\"".$row_opCh["refno"]."\" id=\"".$row_opCh["refno"]."\" onclick=\"checkdb('".$row_opCh["refno"]."', '', 'CRE');\" /></td>
		  </tr>";
		  
		 $sql="insert into tmpbankrecon_deb (ref_no, trn_date, chq_no, name, heading, amt, f, descrip, chk, id, user) values ('".$row_opCh["refno"]."', '".$row_opCh["sdate"]."', '".$row_opCh["chno"]."', '', '', ".$row_opCh["amount"].", '', 'Last Year Balance', '0', 0, '".$_SESSION['UserName']."')";
		$result=mysql_query($sql, $dbacc);
   	}	
	...
	while ($row_strsql = mysql_fetch_array($result_strsql)){
		...
		$ResponseXML .=  "<tr>
		  ...
		  if ($row_strsql["l_flag2"] != "0") { 
				$ResponseXML .=  "<td><input type=\"checkbox\" name=\"".$row_strsql["l_refno"]."\" id=\"".$row_strsql["l_refno"]."\" onclick=\"checkdb('".$row_strsql["l_refno"]."', '".$row_strsql["id"]."', 'CRE');\" checked=\"checked\" /></td>";
				$l_flag2="1";
			} else {
				$ResponseXML .=  "<td><input type=\"checkbox\" name=\"".$row_strsql["l_refno"]."\" id=\"".$row_strsql["l_refno"]."\" onclick=\"checkdb('".$row_strsql["l_refno"]."', '".$row_strsql["id"]."', 'CRE');\" /></td>";
				$l_flag2="0";
			}	

			$ResponseXML .=  " <td>".$row_strsql["id"]."</td></tr>";
			
			$sql="insert into tmpbankrecon_cre (ref_no, trn_date, chq_no, name, heading, amt, f, descrip, chk, id, user) values ('".$row_strsql["l_refno"]."', '".$row_strsql["l_date"]."', '".$row_strsql["chno"]."', '', '', ".$row_strsql["l_amount"].", '".$F."', '".$row_strsql["l_lmem"]."', '".$l_flag2."', ".$row_strsql["id"].", '".$_SESSION['UserName']."')";
		$result=mysql_query($sql, $dbacc); 
   		}
 ...
 if ($_GET["Command"]=="save_customer"){
 	...
 	$sql_rst="select * tmpbankrecon_deb where user='".$_SESSION['UserName']."'";
	$result_rst=mysql_query($sql_rst, $dbacc);
	while ($row_rst = mysql_fetch_array($result_rst)){
		
		if ($row_rst["chk"]=="1"){
			$sql="Update ledger set l_month ='" . date("m", strtotime($_GET["dtpDate"])) . "', recdate='" . $row_rst["f"] . "' where id='" . $row_rst["id"] . "'";
			$result=mysql_query($sql, $dbacc);
			
			$sql="Update ledger set l_flag2 ='1' where id='" . $row_rst["id"] . "'";
			$result=mysql_query($sql, $dbacc);
			
			$sql="update op_cheq set refno='" . trim($row_rst["ref_no"]) . "', recmonth='" . date("m", strtotime($_GET["dtpDate"])) . "'  where refno='" . trim($row_rst["ref_no"])  . "'";
			$result=mysql_query($sql, $dbacc);
	
      
		} else {
			$sql="Update ledger set l_month ='0', recdate='0' where id='" . $row_rst["id"] . "'";
			$result=mysql_query($sql, $dbacc);
			
			$sql="Update ledger set l_flag2 ='0' where id='" . $row_rst["id"] . "'";
			$result=mysql_query($sql, $dbacc);
			
			$sql="update op_cheq set recdate='0',recmonth='0' where refno='" . trim($row_rst["ref_no"])  . "'";
			$result=mysql_query($sql, $dbacc);
		}
	}
	
	
	$sql_rst="select * tmpbankrecon_cre where user='".$_SESSION['UserName']."'";
	$result_rst=mysql_query($sql_rst, $dbacc);
	while ($row_rst = mysql_fetch_array($result_rst)){
		
		if ($row_rst["chk"]=="1"){
			$sql="Update ledger set l_month ='" . date("m", strtotime($_GET["dtpDate"])) . "', recdate='" . $row_rst["f"] . "' where id='" . $row_rst["id"] . "' and l_flag1='CRE' and (chno='" . $row_rst["chq_no"] . "' or l_flag4='CAS')";
			$result=mysql_query($sql, $dbacc);
			
			$sql="Update ledger set l_flag2 ='1' where  id='" . $row_rst["id"] . "' and (chno='" . $row_rst["chq_no"] . "' or l_flag4='CAS')";
			$result=mysql_query($sql, $dbacc);
			
			$sql="update op_cheq set recdate = '" . $row_rst["f"] . "', recmonth = '" . date("m", strtotime($_GET["dtpDate"])) . "'  where  refno = '" . trim($row_rst["ref_no"]) . "' ";
			$result=mysql_query($sql, $dbacc);
	
      
		} else {
			$sql="Update ledger set l_month ='0', recdate='0' where id='" . $row_rst["id"] . "' and l_flag1='CRE' and (chno='" . $row_rst["chq_no"] . "' or l_flag4='CAS')";
			$result=mysql_query($sql, $dbacc);
			
			$sql="Update ledger set l_flag2 ='0' where  id='" . $row_rst["id"] . "' and (chno='" . $row_rst["chq_no"] . "' or l_flag4='CAS')";
			$result=mysql_query($sql, $dbacc);
