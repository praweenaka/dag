    ...
    if ($_GET["TXT_DEBTOT"] > 0) { $mAmount = $_GET["TXT_DEBTOT"]; }
       ...
       if ($_GET["Option1"] == "true") {
          ...
          $mFlag1 = "CRE";
	   	...
	   	$sql_tmp = "Insert into bankentmas(refno, bdate, heading, code, name, amount, type, comcode, invno, vatno, inv_date, bea1) Values ('" . trim($txt_entno) . "', '" . $_GET["dtpDate"] . "', '" . $mHead . "', '" . trim($mCode) . "', '" . trim($mName) . "', '" . $mAmount . "', '" . $mFlag1 . "', '" . $_SESSION['company'] . "', '" . $_GET["txtINVNO"] . "', '" . trim($_GET["txtVATNO"]) . "', '" . $_GET["DTinv_date"] . "', '" . $_GET["txtbea1"] . "')";
		$result_tmp=mysql_query($sql_tmp, $dbacc);
       ...
       if ($_GET["Option1"]=="true") {
	   		 ...
	   		 $l_lmem= $mHead . " De. " . $dCode . " - Ch/No. " . $_GET["txtchno"];
			 
	   		$sql_tmp = "Insert into ledger (l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_head, l_lmem, rights, comcode, chno) Values  ('" . trim($txt_entno) . "', '" . $_GET["dtpDate"] . "', '" . trim($mCode) . "', " . $mAmount . ", 'RTN', '" . $mFlag1 . "', '" . $_GET["TXT_HEADING"] . "', '" .$l_lmem. "', '" . $mUserWrite . "', '" . $_SESSION['company'] . "', '" . trim($_GET["txtchno"]) . "')";
			$result_tmp=mysql_query($sql_tmp, $dbacc);
		
           
        } else {
		
			$sql_tmp = "Insert into ledger (l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_head, l_lmem, rights, comcode, chno) Values  ('" . trim($txt_entno) . "', '" . $_GET["dtpDate"] . "', '" . trim($mCode) . "', " . $mAmount . ", 'BEN', '" . $mFlag1 . "', '" . $_GET["TXT_HEADING"] . "', '" . $mHead . "', '" . $mUserWrite . "', '" . $_SESSION['company'] . "', '" . trim($_GET["txtchno"]) . "')";
			$result_tmp=mysql_query($sql_tmp, $dbacc);
	   ...
	   while ($row_tmp = mysql_fetch_array($result_tmp)){
          ...
          if (($mCode != "") and ($mAmount != 0)) {
		  	...
		  	$sql_tmp = "Insert into bankenttrn(refno, bdate, code, amount, nara, comcode) Values ('" . trim($txt_entno) . "', '" . $_GET["dtpDate"] . "', '" . trim($mCode) . "', " . $mAmount . ", '" . trim($_GET["TXT_HEADING"]) . "', '" . $_SESSION['company'] . "')";
			$result_tmp=mysql_query($sql_tmp, $dbacc);
            ...
            if (is_null(trim($_GET["TXT_HEADING"]))=="false") { $mNara = trim($_GET["TXT_HEADING"]); }
            if (is_null(trim($_GET["TXT_HEADING"]))=="false") { $mHead = trim($_GET["TXT_HEADING"]); }
            if ($_GET["Option1"]=="true") {
				
				$sql_tmp = "Insert into ledger (l_refno, l_date, l_code, l_flag, l_amount, l_flag1, l_flag2, l_flag3, l_lmem,  l_bank, rights, comcode, chno) Values ('" . trim($txt_entno) . "', '" . $_GET["dtpDate"] . "', '" . trim($mCode) . "', 'RTN', " . $mAmount . ", '" . $mFlag1 . "', '0', 'R', '" . $mNara . "', '" . trim($_GET["txtBankCode"]) . "', '" . $mUserWrite . "', '" . $_SESSION['company'] . "', '" . trim($_GET["txtchno"]) . "')";
				$result_tmp=mysql_query($sql_tmp, $dbacc);
				
              
            } else {
				
				$sql_tmp = "Insert into ledger (l_refno, l_date, l_code, l_flag, l_amount, l_flag1, l_flag2, l_flag3, l_lmem,  l_bank, rights, comcode, chno) Values ('" . trim($txt_entno) . "', '" . $_GET["dtpDate"] . "', '" . trim($mCode) . "', 'BEN', " . $mAmount . ", '" . $mFlag1 . "', '0', 'R', '" . $mNara . "',  '" . trim($_GET["txtBankCode"]) . "', '" . $mUserWrite . "', '" . $_SESSION['company'] . "','" . trim($_GET["txtchno"]) . "')";
				$result_tmp=mysql_query($sql_tmp, $dbacc);
