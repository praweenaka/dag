                ...
                if ($mqty >= 50) {
                    ...
                    if ($row_rs1 = mysqli_fetch_array($result_rs1)) {
                        ...
                        while ($row_newinv = mysqli_fetch_array($result_newinv)) {
                            if ($row_newinv['DIS_per'] <= 43.75) {
                                $amount_newinv = $amount_newinv + ($row_newinv['GRAND_TOT'] / (100 - $row_newinv['DIS_per'])) * (43.75 - $row_newinv['DIS_per']);
                            }
                        }
                        ...
                        while ($row_newgrn = mysqli_fetch_array($result_newgrn)) {
                            if ($row_newgrn['DIS_P'] <= 43.75) {
                                $amount_newgrn = $amount_newgrn + ($row_newgrn['GRAND_TOT'] / (100 - $row_newgrn['DIS_P'])) * (43.75 - $row_newgrn['DIS_P']);
                            }
                        }
                    ...
                    if ($row_rs1 = mysqli_fetch_array($result_rs1)) {
                        ...
                        $result_newinv = mysqli_query($GLOBALS['dbinv'], $sql_newinv);
                        while ($row_newinv = mysqli_fetch_array($result_newinv)) {
                            if ($row_newinv['DIS_per'] <= 42.1875) {
                                $amount_newinv = $amount_newinv + ($row_newinv['GRAND_TOT'] / (100 - $row_newinv['DIS_per'])) * (42.1875 - $row_newinv['DIS_per']);
                            }
                        }
                        ...
                        $result_newgrn = mysqli_query($GLOBALS['dbinv'], $sql_newgrn);
                        while ($row_newgrn = mysqli_fetch_array($result_newgrn)) {
                            if ($row_newgrn['DIS_P'] <= 42.1875) {
                                $amount_newgrn = $amount_newgrn + ($row_newgrn['GRAND_TOT'] / (100 - $row_newgrn['DIS_P'])) * (42.1875 - $row_newgrn['DIS_P']);
                            }
                        }
            ...
            if ($row_rsven = mysqli_fetch_array($result_rsven)) {
               if ((($mqty + $qa) >= 100)) {
                ...
                if ($row_rs1 = mysqli_fetch_array($result_rs1)) {
                    ...
                    $result_newinv = mysqli_query($GLOBALS['dbinv'], $sql_newinv);
                    while ($row_newinv = mysqli_fetch_array($result_newinv)) {
                        if ($row_newinv['DIS_per'] <= 41.5) {
                            $amount_newinv = $amount_newinv + ($row_newinv['GRAND_TOT'] / (100 - $row_newinv['DIS_per'])) * (41.5 - $row_newinv['DIS_per']);
                        }
                    }
                    ...
                    $result_newgrn = mysqli_query($GLOBALS['dbinv'], $sql_newgrn);
                    while ($row_newgrn = mysqli_fetch_array($result_newgrn)) {
                        if ($row_newgrn['DIS_P'] <= 41.5) {
                            $amount_newgrn = $amount_newgrn + ($row_newgrn['GRAND_TOT'] / (100 - $row_newgrn['DIS_P'])) * (41.5 - $row_newgrn['DIS_P']);
                        }
                    }
                ...
                if ($row_rs1 = mysqli_fetch_array($result_rs1)) {
                    ...
                    $result_newinv = mysqli_query($GLOBALS['dbinv'], $sql_newinv);
                    while ($row_newinv = mysqli_fetch_array($result_newinv)) {
                        if ($row_newinv['DIS_per'] <= 39.875) {
                            $amount_newinv = $amount_newinv + ($row_newinv['GRAND_TOT'] / (100 - $row_newinv['DIS_per'])) * (39.875 - $row_newinv['DIS_per']);
                        }
                    }
                    ...
                    $result_newgrn = mysqli_query($GLOBALS['dbinv'], $sql_newgrn);
                    while ($row_newgrn = mysqli_fetch_array($result_newgrn)) {
                        if ($row_newgrn['DIS_P'] <= 39.875) {
                            $amount_newgrn = $amount_newgrn + ($row_newgrn['GRAND_TOT'] / (100 - $row_newgrn['DIS_P'])) * (39.875 - $row_newgrn['DIS_P']);
                        }
                    }
                ...
                if ($row_rs1 = mysqli_fetch_array($result_rs1)) {
                    ...
                    $result_newinv = mysqli_query($GLOBALS['dbinv'], $sql_newinv);
                    while ($row_newinv = mysqli_fetch_array($result_newinv)) {
                        if ($row_newinv['DIS_per'] <= 38.25) {
                            $amount_newinv = $amount_newinv + ($row_newinv['GRAND_TOT'] / (100 - $row_newinv['DIS_per'])) * (38.25 - $row_newinv['DIS_per']);
                        }
                    }
                    ...
                    $result_newgrn = mysqli_query($GLOBALS['dbinv'], $sql_newgrn);
                    while ($row_newgrn = mysqli_fetch_array($result_newgrn)) {
                        if ($row_newgrn['DIS_P'] <= 38.25) {
                            $amount_newgrn = $amount_newgrn + ($row_newgrn['GRAND_TOT'] / (100 - $row_newgrn['DIS_P'])) * (38.25 - $row_newgrn['DIS_P']);
                        }
                    }
...
if (trim($_GET["combo1"]) == "ROADSTONE1") {
    $co = 1;
    $ResponseXML .= "
    <tr>
    <td width=\"10%\"  background=\"images/headingbg.gif\" ><font color=\"#FFFFFF\">Code</font></td>
...
if (trim($_GET["combo1"]) == "PRESA") {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $sql = "Select  sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv1   where  DIS_per !=100 and DIS_per !=0 and cus_code = '" . trim($row["cus_code1"]) . "' and s_brand='ULTRA MILE' and  SDATE>='" . $_GET["dtfrom"] . "' and SDATE<='" . $_GET["dtto"] . "' and cancel_m='0' AND type <> 'TBR'";
        $result_m = mysqli_query($GLOBALS['dbinv'], $sql);
        if ($row_m = mysqli_fetch_array($result_m)) {
            $mqty = $mqty + $row_m['qty'];
        }

        $sql = "Select  sum(qty) as qty, sum((s_price * qty) * ((100 - DIS_P)/100)) as grand_tot from viewcrntrn where DIS_p !=100 and DIS_p !=0 and C_CODE='" . trim($row["cus_code1"]) . "' and   Brand='ULTRA MILE' and   SDATE>='" . $_GET["dtfrom"] . "' and SDATE<='" . $_GET["dtto"] . "' and CANCELL='0'  AND s_type <> 'TBR'";
        $result_m = mysqli_query($GLOBALS['dbinv'], $sql);
        if ($row_m = mysqli_fetch_array($result_m)) {
            $mqty = $mqty - $row_m['qty'];
        }
...
if (trim($_GET["combo1"]) == "ULTRA MILE") {
    $ResponseXML .= "<tr>
    <td width=\"10%\"  background=\"images/headingbg.gif\" ><font color=\"#FFFFFF\">Code</font></td>
    <td width=\"20%\"  background=\"images/headingbg.gif\" ><font color=\"#FFFFFF\">Name</font></td>
    <td width=\"10%\"  background=\"images/headingbg.gif\" ><font color=\"#FFFFFF\">37.5%/40%Amt</font></td>
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $sql = "Select  sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv1   where  DIS_per !=100 and DIS_per !=0 and cus_code = '" . trim($row["cus_code1"]) . "' and s_brand='PRESA' and  SDATE>='" . $_GET["dtfrom"] . "' and SDATE<='" . $_GET["dtto"] . "' and cancel_m='0' AND type <> 'TBR'";
        $result_m = mysqli_query($GLOBALS['dbinv'], $sql);
        if ($row_m = mysqli_fetch_array($result_m)) {
            $mqty = $mqty + $row_m['qty'];
        }

        $sql = "Select  sum(qty) as qty, sum((s_price * qty) * ((100 - DIS_P)/100)) as grand_tot from viewcrntrn where DIS_p !=100 and DIS_p !=0 and C_CODE='" . trim($row["cus_code1"]) . "' and   Brand='PRESA' and   SDATE>='" . $_GET["dtfrom"] . "' and SDATE<='" . $_GET["dtto"] . "' and CANCELL='0'  AND s_type <> 'TBR'";
        $result_m = mysqli_query($GLOBALS['dbinv'], $sql);
        if ($row_m = mysqli_fetch_array($result_m)) {
            $mqty = $mqty - $row_m['qty'];
        }
...
//echo $sql_newinv;
        ...
        if ($comm1 >= 0) {
            ...
            $m_flag = date("m/Y", strtotime($_GET["dte_dor"])) . "/UM/" . $row["cus_code"];
        ...
        if ($row_rsven = mysqli_fetch_array($result_rsven)) {
            ...
            if ($mqty >= 100) {
                ...
                if ($row_rs1 = mysqli_fetch_array($result_rs1)) {
                    ...
                    $result_newinv = mysqli_query($GLOBALS['dbinv'], $sql_newinv);
                    while ($row_newinv = mysqli_fetch_array($result_newinv)) {
                        if ($row_newinv['DIS_per'] <= $dis) {
                            $amount_newinv = $amount_newinv + ($row_newinv['GRAND_TOT'] / (100 - $row_newinv['DIS_per'])) * ($dis - $row_newinv['DIS_per']);
                        }
                    }
                    ...
                    $result_newgrn = mysqli_query($GLOBALS['dbinv'], $sql_newgrn);
                    while ($row_newgrn = mysqli_fetch_array($result_newgrn)) {
                        if ($row_newgrn['DIS_P'] <= $dis) {
                            $amount_newgrn = $amount_newgrn + ($row_newgrn['GRAND_TOT'] / (100 - $row_newgrn['DIS_P'])) * ($dis - $row_newgrn['DIS_P']);
                        }
                    }
            ...
            if ($row_tmp["amount_c"] > 0) {
                for ($X = 1; $X <= 2; $X++) {
                    ...
                    if (trim($_GET["combo1"]) == "ULTRA MILE") {
                        ...
                        $m_flag = date("m/Y", strtotime($_GET["dte_dor"])) . "/UM/" . trim($row_tmp['code1']);
                    }
                    ...
                    $txt_remark = "Additional Trade Discounts for ".trim($_GET["combo1"])." month of " . date("m/Y", strtotime($_GET["dte_dor"]));
                    ...
                    if ($X == 1) {
                        ...
                        $sql_cbal = "insert into c_bal (c_code1,REFNO, SDATE, trn_type, CUSCODE, AMOUNT, BALANCE, brand, DEP, SAL_EX, DEV, vatrate,sdate1,block) values('" . trim($row_tmp['code1']) . "','" . trim($txtrefno) . "', '" . date("Y-m-d") . "', 'CNT', '" . trim($txt_cuscode) . "', " . $txtamount . ", " . $txtamount . ", '" . trim($_GET["combo1"]) . "', '" . $m_dep . "', '" . $m_rep . "', '0', '" . $mvatrate . "','" . date("Y-m-d") . "','1')";
                        echo $sql_cbal;
                        $result_cbal = mysqli_query($GLOBALS['dbinv'], $sql_cbal);
                    }

                    if ($X == 2) {
                        $sql_cbal = "insert into c_bal (c_code1,REFNO, SDATE, trn_type, CUSCODE, AMOUNT, BALANCE, brand, DEP, SAL_EX, DEV, vatrate,sdate1,block) values('" . trim($row_tmp['code1']) . "','" . trim($txtrefno) . "', '" . date("Y-m-d") . "', 'CNT', '" . trim($txt_cuscode) . "', " . $txtamount . ", " . $txtamount . ", 'ROADSTONE CHINA', '" . $m_dep . "', '" . $m_rep . "', '0', '" . $mvatrate . "','" . date("Y-m-d") . "','1')";
                        echo $sql_cbal;
                        $result_cbal = mysqli_query($GLOBALS['dbinv'], $sql_cbal);
                    }
                    ...
                    if ($X == 2) {
                        $sql_cred = "Insert into cred (C_REFNO, C_DATE, C_INVNO, C_CODE, C_PAYMENT, C_REMARK, C_SALEX, Brand, DEV, C_SETINV,sdate1) values ('" . trim($txtrefno) . "', '" . date("Y-m-d") . "', '" . trim($txt_invno) . "', '" . trim($txt_cuscode) . "', " . $txtamount . ", '" . trim($txt_remark) . "', '" . trim($m_rep) . "', 'ROADSTONE CHINA', '0', '" . trim($m_flag) . "','" . date("Y-m-d") . "')";
                        echo $sql_cred;
                        $result_cred = mysqli_query($GLOBALS['dbinv'], $sql_cred);
                    }
