...
//echo $sql;	
        while ($row = mysqli_fetch_array($result)) {
            ...
            $sql_40 = "Select  sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv   where c_code1 = '" . trim($row["cus_code"]) . "' and s_brand='" . trim($_GET["combo1"]) . "' and  month(SDATE)='" . date("m", strtotime($_GET["dte_dor"])) . "' and year(SDATE)='" . date("Y", strtotime($_GET["dte_dor"])) . "' and DIS_per = '30' and cancel_m='0'";
            $result_40 = mysqli_query($GLOBALS['dbinv'], $sql_40);

            $sql_43 = "Select  sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv   where c_code1 = '" . trim($row["cus_code"]) . "' and s_brand='" . trim($_GET["combo1"]) . "' and  month(SDATE)='" . date("m", strtotime($_GET["dte_dor"])) . "' and year(SDATE)='" . date("Y", strtotime($_GET["dte_dor"])) . "' and dis_per <> '32.5' AND DIS_PER <> '35' and cancel_m='0' ";
            $result_43 = mysqli_query($GLOBALS['dbinv'], $sql_43);
            ...
            $sql_37 = "Select  sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv   where c_code1 = '" . trim($row["cus_code"]) . "' and s_brand='" . trim($_GET["combo1"]) . "' and  month(SDATE)='" . date("m", strtotime($_GET["dte_dor"])) . "' and year(SDATE)='" . date("Y", strtotime($_GET["dte_dor"])) . "' and DIS_per = '32.5' and cancel_m='0'  ";
            $result_37 = mysqli_query($GLOBALS['dbinv'], $sql_37);
            ...
            if ($row_rsven = mysqli_fetch_array($result_rsven)) {
                ...
                if (($mqty >= 50) and ($mqty != 0)) {
                    ...
                    if ($row_rs1 = mysqli_fetch_array($result_rs1)) {
                        ...
                        $ResponseXML .= "<td>Not Saved</td>
												   <td></td>
												   <td>" . number_format((($amount_40 - $rtn_40) / 70 * 5), 2, ".", ",") . "</td>
												   <td></td>
												   <td><input type=\"checkbox\" name=\"" . $chk . "\" id=\"" . $chk . "\" onclick=\"changechk('" . $i . "', '" . $code . "');\"  /></td>";

                        $crnno = "Not Saved";
                        $txndate = "";
                        $amount = (($amount_40 - $rtn_40) / 70 * 5);
                        $amount_c = 0;
                        $status = "";
...
//echo $sql;			
        while ($row = mysqli_fetch_array($result)) {
            ...
            $sql_40 = "Select c_code1, sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv   where  c_code1 = '" . trim($row["cus_code"]) . "' and s_brand='" . trim($_GET["combo1"]) . "' and  month(SDATE)='" . date("m", strtotime($_GET["dte_dor"])) . "' and year(SDATE)='" . date("Y", strtotime($_GET["dte_dor"])) . "' and (DIS_per = 45 or dis_per = 47.5) and cancel_m='0' group by c_code1";
            $result_40 = mysqli_query($GLOBALS['dbinv'], $sql_40);
            ...
            if (date("Y", strtotime($_GET["dte_dor"])) >= 2014) {
                $sql_RS_40_C = "Select c_code1,sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv   where c_code1 = '" . trim($row["cus_code"]) . "' and s_brand='ROADSTONE CHINA' and  month(SDATE)=" . date("m", strtotime($_GET["dte_dor"])) . " and year(SDATE)=" . date("Y", strtotime($_GET["dte_dor"])) . " and (DIS_per = 45 or DIS_per = 47.5) and cancel_m='0' group by c_code1";
