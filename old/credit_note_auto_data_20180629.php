...
//echo $sql;			
    ...
    if (trim($_GET["combo1"]) == "PRESA") {
        ...
        while ($row = mysqli_fetch_array($result)) {
			...
			$mqty =0;
			$sql = "Select  sum(QTY) as qty, sum((PRICE * QTY) * ((100 - DIS_per)/100)) as amount from viewinv1   where  DIS_per !=100 and DIS_per !=0 and cus_code = '" . trim($row["cus_code1"]) . "' and s_brand='" . trim($_GET["combo1"]) . "' and  month(SDATE)='" . date("m", strtotime($_GET["dte_dor"])) . "' and year(SDATE)='" . date("Y", strtotime($_GET["dte_dor"])) . "' and cancel_m='0' AND type <> 'TBR'";
            $result_m = mysqli_query($GLOBALS['dbinv'], $sql);
            if ($row_m = mysqli_fetch_array($result_m)) {
                $mqty = $mqty + $row_m['qty'];
            }
            
            $sql = "Select  sum(qty) as qty, sum((s_price * qty) * ((100 - DIS_P)/100)) as grand_tot from viewcrntrn where DIS_p !=100 and DIS_p !=0 and C_CODE='" . trim($row["cus_code1"]) . "' and   Brand='" . trim($_GET["combo1"]) . "' and   month(SDATE)='" . date("m", strtotime($_GET["dte_dor"])) . "' and year(SDATE)='" . date("Y", strtotime($_GET["dte_dor"])) . "' and CANCELL='0'  AND s_type <> 'TBR'";
            $result_m = mysqli_query($GLOBALS['dbinv'], $sql);
            if ($row_m = mysqli_fetch_array($result_m)) {
                $mqty = $mqty - $row_m['qty'];
            }
