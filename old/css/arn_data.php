...
TYPE, brand) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$_GET["orderno1"]."', '".$_GET["lc_no"]."', '".$_GET["pi_no"]."', '".$_GET["country"]."', '".$_GET["supplier_code"]."', '".$_GET["supplier_name"]."', '".$_GET["textarea"]."', '".$_GET["department"]."', '".$_GET["total_value"]."', '".$_GET["dte_dor"]."', '".$_GET["purtype"]."', '".$_GET["brand"]."')";
				//echo $sql1;
				$result1 =$db->RunQuery($sql1);
			}
			...
			while ($i<$_GET["count"]){
				...
				if ($QTYINHAND>0){
					$m_totval=(($QTYINHAND*$acc_cost_c)+($_GET[$qty_name]*$_GET[$cost_name]))/$m_qty;
				} else {
 ...
 Cost_c, acc_cost, acc_cost_c, brand, vatrate, DISCOUNT, ret_qty, cost_seling, cost_margin, CANCEL, soldqty, days) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$_GET[$itemcode_name]."', '".$_GET[$itemname_name]."', ".$cost.", ".$margin_name.", ".$_GET[$selling_name].", ".$_GET[$qty_name].", ".$_GET[$fob_name].", '".$_GET["department"]."', ".$QTYINHAND.", ".$_GET[$ord_qty_name].", ".$COST_mas.", '".$acc_cost."', '".$acc_cost_c."', '".$_GET["brand"]."', '".$vatrate."', 0, 0, 0, 0, '0', '', 0)";
 			//echo $sql1;
				$result1 =$db->RunQuery($sql1);
				...
				if ($_GET["purtype"]== "Local"){
					$sql1="update s_mas set COST=".$m_totval." where  STK_NO='".$_GET[$itemcode_name]."'";
					$result1 =$db->RunQuery($sql1);
				}
				
				$margin=($_GET[$selling_name]-$m_totval)/$m_totval*100;
				
				$sql1="update s_mas set acc_cost=".$m_totval.", SELLING='".$_GET[$selling_name]."', AR_selling='".$_GET[$selling_name]."', MARGIN ='".$margin."', QTYINHAND=QTYINHAND+".$_GET[$qty_name]." where  STK_NO='".$_GET[$itemcode_name]."'";
				$result1 =$db->RunQuery($sql1);
				//echo $sql1;
				...
				$sql1="insert into s_trn(STK_NO, SDATE, QTY, LEDINDI, REFNO, DEPARTMENT, seri_no, Dev, SAL_EX, ACTIVE, DONO) values ('".$_GET[$itemcode_name]."', '".$_GET["invdate"]."', '".$_GET[$qty_name]."', 'ARN', '".$_GET["invno"]."', '".$_GET["department"]."', '', '".$_SESSION['dev']."', '', '1', '')";
				//echo $sql1;
				$result1 =$db->RunQuery($sql1);
			...
			while($row = mysql_fetch_array($result)){	
				...
				if (($row['REC_QTY'] > 0) and ($row['acc_cost'] > 0) and ($row['SELLING'] > 0)) {
             	...
             	$ResponseXML .= "<tr>                              
                             <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$itemcode." id=".$itemcode."   value='".$row['STK_NO']."' class=\"txtbox\" disabled=\"disabled\"/></td>
							  <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$itemname." id=".$itemname."  value='".$row['DESCRIPT']."' class=\"txtbox\" disabled=\"disabled\"/></td>
							  <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$ord_qty." id=".$ord_qty."  value='".$row['O_QTY']."' class=\"txtbox\" disabled=\"disabled\"/></td>
							
							 <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$fob." id=".$fob." value='".$row['FOB']."'  class=\"txtbox\"/></td>
							 <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$qty." id=".$qty." value='".$row['REC_QTY']."'  class=\"txtbox\" onBlur=\"cal_subtot('".$i."', '".$mcou."');\"/></td>
							 <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$cost." id=".$cost." value='".$row['acc_cost']."'  class=\"txtbox\" onBlur=\"cal_subtot('".$i."', '".$mcou."');\"/></td>
							 <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$selling." id=".$selling." value='".$row['SELLING']."'  class=\"txtbox\"/></td>
							 <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$margin." id=".$margin." value='".number_format($margine_val, 2, ".", ",")."'  class=\"txtbox\" disabled=\"disabled\"/></td>
							 <td bgcolor=\"#222222\" ><input type=\"text\" size=\"15\" name=".$subtotal." id=".$subtotal." value='".number_format($stot, 2, ".", ",")."'  class=\"txtbox\" disabled=\"disabled\"/></td>
							</tr>";
