...
{   
		...
		//} else {
			...
			url=url+"&chkcash_disc="+document.getElementById('chkcash_disc').value;
...
function result_save_crn()
{
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		//	alert("Already Exists");	
		//}
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("msg_exist");	
		if (XMLAddress1[0].childNodes[0].nodeValue != 0){
			alert(XMLAddress1[0].childNodes[0].nodeValue);
		}
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("msg_saved");	
		alert(XMLAddress1[0].childNodes[0].nodeValue);
		setTimeout("location.reload(true);",500);
