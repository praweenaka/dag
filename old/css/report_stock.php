...
function cost()
{
	...
	if ($_GET["department"] == "All"){
        if ($_GET["brand"] != "All") {$sql = "select * from s_mas where BRAND_NAME='".$_GET["brand"]."' and QTYINHAND>0 and COST>0 ORDER BY BRAND_NAME, GROUP1, GROUP2, model "; }
        if ($_GET["brand"] == "All") {$sql = "select * from s_mas where QTYINHAND!=0 and COST!=0 order by BRAND_NAME, GROUP1, GROUP2, model ";}
    }
	
    if ($_GET["department"] != "All"){
        if ($_GET["brand"] != "All") { $sql = "select * from viewsubmas where BRAND_NAME='".$_GET["brand"]."' and QTYINHAND!=0 and COST!=0 ORDER BY BRAND_NAME, GROUP1, GROUP2, model "; }
        if ($_GET["brand"]	== "All") { $sql = "select * from viewsubmas where QTYINHAND>0 and COST>0 ORDER BY BRAND_NAME, GROUP1, GROUP2, model  "; }
    }
	
	
	echo "<center><table border=1><tr>
		<th>Item</th><th>Description</th><th>Part No</th><th>Qty in hand </th><th>Cost</th></tr>";
	...
	while ($rows = mysql_fetch_array($result)){
		...
		echo "<tr><td>".$rows["STK_NO"]."</td><td>".$rows["DESCRIPT"]."</td><td>".$rows["PART_NO"]."</td><td align=\"right\">".number_format($rows["QTYINHAND"], 0, ".", ",")."</td><td align=\"right\">".number_format($rows["COST"], 2, ".", ",")."</td></tr>";
	}
...
function selling()
{
	...
	$db = new DBConnector();
        ...
        echo "<center><table border=1><tr>
		<th>Item</th><th>Description</th><th>Part No</th><th>Qty in hand </th><th>Total</th></tr>";
	...
	while ($rows = mysql_fetch_array($result)){
		...
		echo "<tr><td>".$rows["STK_NO"]."</td><td>".$rows["DESCRIPT"]."</td><td>".$rows["PART_NO"]."</td><td align=\"right\">".number_format($rows["QTYINHAND"], 0, ".", ",")."</td><td align=\"right\">".number_format($rows["QTYINHAND"]*$rows["COST"], 2, ".", ",")."</td></tr>";
	}
...
function print_rep()
{
	...
	while ($rows = mysql_fetch_array($result)){
		...
		echo "<tr><td>".$rows["STK_NO"]."</td><td>".$rows["DESCRIPT"]."</td><td>".$rows["PART_NO"]."</td><td align=\"right\">".number_format($rows["QTYINHAND"], 0, ".", ",")."</td></tr>";
	}
...
if ($_GET["department"]=="All"){
	...
	if ($_GET["chkitem"]!="on") {
    	...
    	if ($_GET["brand"] != "All")  {$sql = "select * from viewsubmas where BRAND_NAME='".$_GET["brand"]."' and STO_CODE='".$_GET["department"]."' and QTYINHAND!=0  ORDER BY GROUP1, GROUP2, model ";}
