	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
								$sql = mysql_query("SELECT * from s_salma where REF_NO like  '$letters%' limit 50") or die(mysql_error());
							...
							while($row = mysql_fetch_array($sql)){
								...
								$stname = "inv_mast";
							$ResponseXML .= "<tr>
                           	  <td  onclick=\"invno('$REF_NO');\">".$row['REF_NO']."</a></td>
                              <td  onclick=\"invno('$REF_NO');\">".$row['CUS_NAME']."</a></td>
			...
			$ResponseXML .= "<salesdetails>";
				...
				while($row = mysql_fetch_array($sql_data)){
			 	...
			 	$ResponseXML .= "<tr>
							 ...
							 <td ><input type=\"text\" size=\"10\" name=".$stkno."  disabled id=".$stkno." value='".$row['STK_NO']."' class=\"txtbox\"/></td>
							 <td ><input type=\"text\" size=\"30\" name=".$descript."  disabled id=".$descript." value='".$row['DESCRIPT']."' class=\"txtbox\"/></td>
							 <td ><input type=\"text\" size=\"10\" name=".$price."  disabled id=".$price." value=".number_format($row['PRICE'], 2, ".", ",")." class=\"txtbox\"/></td>
							 <td  ><input type=\"text\" size=\"10\" name=".$qty."  disabled id=".$qty." value=".$row['QTY']." class=\"txtbox\"/></td>";
						
						$totqty=0;
						$sql_ret_mast = mysql_query("Select * from s_crnma where INVOICENO='".$inv."'") or die(mysql_error());
						while($row_ret_mast = mysql_fetch_array($sql_ret_mast)){ 	
							$sql_ret = mysql_query("Select sum(QTY) as totqty from s_crntrn where REF_NO='".$row_ret_mast["REF_NO"]."' and STK_NO='".$row['STK_NO']."'") or die(mysql_error());
							$row_ret = mysql_fetch_array($sql_ret);
							$totqty=$totqty+$row_ret["totqty"];
						}	
							 $ResponseXML .= "<td  ><input type=\"text\" size=\"10\" name=".$preretqty."  disabled id=".$preretqty." value=".$totqty." class=\"txtbox\"/></td>";
							
							 $ResponseXML .= "<td  ><input type=\"text\" size=\"10\" name=".$retqty." id=".$retqty."  class=\"txtbox\" onKeyPress=\"ret_deduct('".$i."',event, '".$mcou."');\"/></td>
							 <td  ><input type=\"text\" size=\"10\" name=".$disc." id=".$disc." value=".$row['DIS_per']." class=\"txtbox\"/></td>
							 <td  ><input type=\"text\" size=\"10\" name=".$subtot." id=".$subtot." value='0' class=\"txtbox\"/></td>
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			$ResponseXML .= "<sales_table><![CDATA[ <table><tr>
				...
				//	echo "Insert into tmp_inv_data(str_invno, str_code, str_description, cur_rate, cur_qty, dis_per,cur_discount, cur_subtot, brand) values ( '".$inv."', '".$row['STK_NO']."', '".$row['DESCRIPT']."', ".$row['PRICE'].", ".$row['QTY'].", ".$row['DIS_per'].", ".$row['DIS_rs'].", ".$row["subtot"].", '".$row['BRAND']."')";
					...
					$sql_tmp = mysql_query("Insert into tmp_inv_data(str_invno, str_code, str_description, cur_rate, cur_qty, dis_per,cur_discount, cur_subtot, brand, ad, tmp_no) values ( '".$inv."', '".$row['STK_NO']."', '".$row['DESCRIPT']."', ".$row['PRICE'].", ".$row['QTY'].", ".$row['DIS_per'].", ".$row['DIS_rs'].", ".$subtot.", '".$row['BRAND']."', '".$row['ad']."', '".$_SESSION["tmp_no_salinv"]."')") or die(mysql_error());
