	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
								$sql = mysql_query("SELECT * from s_ordmas where cancel='0' and REFNO like  '$letters%' order by S_date desc  limit 50") or die(mysql_error());
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_ordmas where cancel='0' and SUP_NAME like  '$letters%' order by S_date desc  limit 50") or die(mysql_error()) or die(mysql_error());
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysql_query("SELECT * from s_ordmas where cancel='0' and SUP_NAME like  '$letters%' order by S_date desc  limit 50") or die(mysql_error()) or die(mysql_error());
