...
if ($_GET["Command"] == "proces") {
    ...
    $sql_rssslma = "select * from view_s_salma_brand_mas_incentive where  month(sdate)='" . date("m", strtotime($_GET["DTPicker1"])) . "' and  year(sdate)='" . date("Y", strtotime($_GET["DTPicker1"])) . "' and c_CODE = '" . $_GET["txt_cuscode"] . "' and sfrom <='" . $dtfrom . "' and sto >='" . $dtfrom . "' and inc_type='" . $_GET['cmbtype'] . "' and cancell='0' and totpay1 = '1'";
...
//echo $sql_rsIncen;
    ...
    $sql_RScbal = "select * from view_cbal_brand_incentive where month(SDATE)='" . date("m", strtotime($_GET["DTPicker1"])) . "' and  year(SDATE)='" . date("Y", strtotime($_GET["DTPicker1"])) . "' and CANCELL='0'and CUSCODE='" . trim($_GET["txt_cuscode"]) . "' AND trn_type != 'ARN' and trn_type != 'REC' and trn_type != 'DGRN' and flag1 != '1'  and sfrom <='" . $dtfrom . "' and sto >='" . $dtfrom . "' and inc_type='" . $_GET['cmbtype'] . "' order by SDATE";
