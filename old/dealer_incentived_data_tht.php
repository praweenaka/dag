...
if (isset($_POST["Command"])) {
    ...
    if ($_POST["Command"] == "calculate_ins") {
        ...
        // getting commition % for sale
            ...
            $sql_rsper = "Select * from intper where incen_year = 20161 and class='" . $_POST['cmbtype'] . "' and traget < '" . (($txttot1_w - $txt_grn_w)) . "' order by traget desc ";
    ...
    if ($row_rs = mysqli_fetch_array($result_rs)) {
        ...
        if (date("Y", strtotime($_POST["DTPicker1"])) >= 2014) {
            //if (trim($_POST["cmbtype"]) == "") {
                $sql_rsper = "Select * from intper where incen_year = 20161 and class='".$_GET['cmbtype']."' and traget < '" . (($txttot1 - $txt_grn) / ($txt_vat + 100) * 100) . "' order by traget desc ";
           // }
        } else {
            if (trim($_POST["cmbtype"]) == $_GET['cmbtype']) {
                $sql_rsper = "Select * from intper where incen_year = 20161 and class='" . $_GET['cmbtype'] . "' and traget < '" . (($txttot1 - $txt_grn) / ($txt_vat + 100) * 100) . "' order by traget desc ";
            }
        }
        if (trim($_POST["cmbtype"]) == "Battery") {
...
if ($_GET["Command"] == "proces") {
    ...
    $sql_rssslma = "select * from s_salma where  brand ='" . $_GET['cmbtype'] . "' and month(sdate)='" . date("m", strtotime($_GET["DTPicker1"])) . "' and  year(sdate)='" . date("Y", strtotime($_GET["DTPicker1"])) . "' and c_CODE = '" . $_GET["txt_cuscode"] . "' and cancell='0' and totpay1 = '1'";
...
//echo $sql_rsIncen;
    ...
    $sql_RScbal = "select * from c_bal where brand ='" . $_GET['cmbtype'] . "' and month(SDATE)='" . date("m", strtotime($_GET["DTPicker1"])) . "' and  year(SDATE)='" . date("Y", strtotime($_GET["DTPicker1"])) . "' and CANCELL='0'and CUSCODE='" . trim($_GET["txt_cuscode"]) . "' AND trn_type != 'ARN' and trn_type != 'REC' and trn_type != 'DGRN' and flag1 != '1' order by SDATE";
...
////Private Sub Auto_cal()
    ...
    while ($i < $TypeGrid1_count) {
        ...
        $sql_rsper = "Select * from intper where incen_year = 20161 and class='".$_GET['cmbbrand']."' and traget < '" . (($txttot1 - $txt_grn) / ($txt_vat + 100) * 100) . "' order by traget desc ";
