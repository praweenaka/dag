	...
	if($row = mysqli_fetch_array($result)){
		...
		/*	$sql="Select SPINV, CRE_INV_NO, CAS_INV_NO from invpara";
		 ...
		 // if ($_SESSION['company']=="THT"){	
			...
			$sql="Select STOINV, STOINV_CODE from invpara";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			//if ($_SESSION['dev']=="1"){
				 				
				$tmpinvno="000000".($row["STOINV"]+1);
				$lenth=strlen($tmpinvno);
				
				
				
				$invno=$row["STOINV_CODE"]."/".substr($tmpinvno, $lenth-5); 
				
				//trim("CRI/ ").substr($tmpinvno, $lenth-6)."/5";
	...
	if($_GET["Command"]=="add_tmp")
		{
			...
			('".$_GET['invno']."', '".$_GET['itemcode']."', '".$_GET['item']."', ".$rate.", ".$qty.", ".$_GET["discountper"].", ".$discount.", ".$subtotal.", ".$rate.", '".$_GET['brand']."', '".$_SESSION["tmp_no_salinv"]."') ";
...
//echo "SELECT * FROM s_invcheq WHERE che_date>'".$_GET["invdate"]."' AND cus_code='".trim($cuscode)."' and trn_type='REC' and sal_ex='".trim($salesrep)."'";
 ...
 $d=date("Y-m-d");
		...
		$sql_rssal = mysqli_query($GLOBALS['dbinv'],"Select sum(GRAND_TOT - TOTPAY) as out1 from s_salma where C_CODE = '" . trim($cuscode) . "' and (SDATE < '" . $date . "' or SDATE = '" . $date . "') and GRAND_TOT - TOTPAY > 1 and CANCELL = '0'") or die(mysqli_error());
	...
	if($_GET["Command"]=="del_item")
		{
			...
			$ResponseXML .= "<sales_table><![CDATA[ <table><tr>
                              ...
                              <td width=\"300\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Description</font></td>
                              <td width=\"100\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Rate</font></td>
							  <td width=\"100\"  background=\"images/headingbg.gif\"></td>
                              <td width=\"100\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Qty</font></td>
                              <td width=\"100\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Discount</font></td>
                              <td width=\"100\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Sub Total</font></td>
							   <td></td><td width=\"50\">AD</td>
...
//==============================================================
			...
			$sql="update vendor set Over_DUE_IG_Date='" . $date1 . "'  where code='" . trim($_GET["customercode"]) . "'";
			$result=mysqli_query($GLOBALS['dbinv'],$sql);	
	...
	//if ($_GET["brand"]=="TRANSKING" and $d > 35){
	   ...
	   //==============Update credit limit==========================================
        ...
        //M_STOCODE = Trim(Mid(Me.com_dep, 1, 5))
				...
				if ($results_submas!=1){ $sql_status=1; }		
			...
			//if ($_SESSION['dev']=="0"){
				$sqlpara="update invpara set STOINV=STOINV+1";
				$resultpara=mysqli_query($GLOBALS['dbinv'],$sqlpara);	
				if ($resultpara!=1){ $sql_status=1; }		
			//}
			...
			if($rowbrand = mysqli_fetch_array($resultbrand)){
				...
				if ($resultbr_trn!=1){ $sql_status=1; }		
			}
			...
			if ($_SESSION['dev']!="1"){
				$sqlbr_trn="update  s_salma set Account = 'PRINT', DES_CAT='Y'   WHERE REF_NO='" . trim($invno) . "'";
				$resultbr_trn=mysqli_query($GLOBALS['dbinv'],$sqlbr_trn);	
				if ($resultbr_trn!=1){ $sql_status=1; }		
			}
