...
function caldis(i)
{
	...
	var Amount = "settleamt" + i;
	
	var Incen_per = "Incen_per" + i;
   	document.getElementById(Incen_val).value = document.getElementById(Amount).value *  (document.getElementById(Incen_per).value /(100-(document.getElementById(Incen_per).value)));
...
function set_month()
{
	xmlHttp=GetXmlHttpObject();
			...
			xmlHttp.onreadystatechange=result_set_month;
			xmlHttp.open("GET",url,true);
			xmlHttp.send(null);
}

function result_set_month()
{
	var XMLAddress1;
	...
	if (document.getElementById('txtrefno').value==""){
		...
		if( document.getElementById('lock').innerHTML==""){
			...
			while (i<document.getElementById('mcou').value){
				...
				url=url+"&"+disc1+"="+document.getElementById(disc1).value;
...
function passinvresult_crn_form()
{
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		opener.document.form1.mcou.value=0;
