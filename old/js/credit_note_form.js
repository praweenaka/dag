...
function invno_check_crn(invno, stname, amt)
{   
			...
			if (xmlHttp==null)
			{
				...
				var url="credit_note_form_data.php";			
				url=url+"?Command="+"invno_check_crn";
				url=url+"&invno="+invno;
				url=url+"&amt="+amt;
				//url=url+"&txtrefno="+opener.document.form1.txtrefno.value;
				//url=url+"&mcou="+opener.document.form1.mcou.value;
				...
				//alert(url);
				xmlHttp.onreadystatechange=passinvresult_check_crn;
				xmlHttp.open("GET",url,true);
				xmlHttp.send(null);
...
function passinvresult_check_crn()
{
	var XMLAddress1;
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("txt_invno");
		opener.document.form1.invno.value = XMLAddress1[0].childNodes[0].nodeValue;
		...
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("txt_totpay");
		...
		//alert("ok");
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("txtinvdate");
		...
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("chkcashdis");
		var chkcashdis = XMLAddress1[0].childNodes[0].nodeValue;
		...
		//XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("txt_remark");
		//opener.document.form1.remarks.value = XMLAddress1[0].childNodes[0].nodeValue;
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("txtamount");
		opener.document.form1.amount.value  = XMLAddress1[0].childNodes[0].nodeValue;
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("txt_frmno");
...
function cal_incentive(i)
{
	...
	//alert(document.getElementById(Incen_val).value);
	var j=1;
	var tot=0;
...
function select_list(mstatus, stname)
{
	xmlHttp=GetXmlHttpObject();
			if (xmlHttp==null)
			{
