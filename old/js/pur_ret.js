...
function cal_subtot(cost, mcount)
{
	alert(cost);
	alert(mcount);
	var retqty='retqty'+count;
	
	document.getElementById(retqty).value=parseFloat(cost)*parseFloat(document.getElementById(retqty).value);
...
function update_list_arn(stname)
{   
...
function ret_deduct(key, e, mcou)
{	
   ...
   if(e.keyCode==13){
	   ...
	   valprice=parseFloat(valprice.replace(/,/gi, ""));
		...
		if ((valqty-valpreret)>valret){
			document.getElementById(subtotbox).value=valret*valprice;
			...
			while (i<=mcou){
				...
				valdisc=valdisc+parseFloat(document.getElementById(discitembox).value)*parseFloat(document.getElementById(subtotitembox).value)/100;
				i=i+1;
			}
			document.getElementById('subtot').value=valsubtot;
			document.getElementById('totdiscount').value=valdisc;
			document.getElementById('invtot').value=parseFloat(document.getElementById('subtot').value)-parseFloat(document.getElementById('totdiscount').value);
		} else {
			document.getElementById(subtotbox).value=0;
		}
   }
}
...
function assign_invno(){
	...
	document.getElementById('pur_ret_no').value = XMLAddress1[0].childNodes[0].nodeValue;
