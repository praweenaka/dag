...
{
    var overdue_amt = document.getElementById(overdue).value;
    overdue_amt = overdue_amt.replace(/,/g, '');
    overdue_amt = parseFloat(overdue_amt);
    overdue_amt = overdue_amt.toFixed(2);

    var chq_pay_amt = document.getElementById(chq_pay).value;
    if (chq_pay_amt == "") {
        chq_pay_amt = "0.00";
    }
    chq_pay_amt = chq_pay_amt.replace(/,/g, '');
    chq_pay_amt = parseFloat(chq_pay_amt);
    chq_pay_amt = chq_pay_amt.toFixed(2);

    if (Number(overdue_amt) >= Number(chq_pay_amt)) {
...
function save_crec()
{
    ...
    if (document.getElementById("invno").value == "") {
        ...
        url = url + "&cashttdate=" + document.getElementById('cashttdate').value;
