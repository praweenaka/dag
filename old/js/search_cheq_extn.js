...
function select_list(mstatus)
{
	xmlHttp=GetXmlHttpObject();
			if (xmlHttp==null)
			{
				...
				var url="search_chq_extn_data.php";			
				url=url+"?Command="+"select_list";
				url=url+"&chqno="+document.getElementById('invno').value;
...
function chq_extn(cheque_no, ref_no)
{   
			...
			if (xmlHttp==null)
			{
				...
				window.opener.document.getElementById('ded').value = "--";
				
				var url="search_chq_extn_data.php";			
				url=url+"?Command="+"extend";
				url=url+"&txtChequeNo="+cheque_no;
				//url=url+"&txtChequeNo="+cheque_no;
				url=url+"&ref_no="+ref_no;
...
function passinvresult_chq_extn()
{
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("autho");
		var autho = XMLAddress1[0].childNodes[0].nodeValue;
		if (autho=="1"){
			opener.document.form1.txtchexdate.disabled=false;
		} else {
			opener.document.form1.txtchexdate.disabled=true;	
		}
		...
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("ch_exdate");
		opener.document.form1.txtchexdate.value = XMLAddress1[0].childNodes[0].nodeValue;
					
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("approved");
		opener.document.form1.txtapproved.value = XMLAddress1[0].childNodes[0].nodeValue;
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("acc_approved");
		opener.document.form1.Textposted.value = XMLAddress1[0].childNodes[0].nodeValue;
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("sales_table");
		window.opener.document.getElementById('itemdetails').innerHTML = XMLAddress1[0].childNodes[0].nodeValue;
		
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("ded");
		opener.document.form1.ded.value = XMLAddress1[0].childNodes[0].nodeValue;
		
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("doc_feed");
		if (XMLAddress1[0].childNodes[0].nodeValue =="1") {
			
				window.opener.document.getElementById('dedlb1').disabled=false;
				window.opener.document.getElementById('ded').disabled=false;
	
			
		} else {
				window.opener.document.getElementById('dedlb1').disabled=true;
				window.opener.document.getElementById('ded').disabled=true;
