...
{   
			...
			var url="search_item_pass_stk_adj_data.php";			
			url=url+"?Command="+"pass_stk_adj";
			url=url+"&refno="+refno;
		
	
			xmlHttp.onreadystatechange=pass_stk_adj_result;
			xmlHttp.open("GET",url,true);
			xmlHttp.send(null);
}

function pass_stk_adj_result()
{
	var XMLAddress1;
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		opener.document.form1.spinst.value = XMLAddress1[0].childNodes[0].nodeValue;
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("trn_type");	
		var ttype= XMLAddress1[0].childNodes[0].nodeValue;
		
		if (ttype=="IIN"){
			opener.document.form1.Addition.checked=true; 
		} else if (ttype=="IOU"){
			opener.document.form1.Deduction.checked=true; 
		}
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("dep");	
		opener.document.form1.dep.value = XMLAddress1[0].childNodes[0].nodeValue;
