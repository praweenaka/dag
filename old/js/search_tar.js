   ...
   if(e.keyCode==13){
	   ...
	   valprice=parseFloat(valprice.replace(/,/gi, ""));
		...
		if ((valqty-valpreret)>=valret){
			...
			var before_vat=parseFloat(document.getElementById('subtot').value)-parseFloat(document.getElementById('totdiscount').value);
			var vat=before_vat*0.12;
			
			if (document.getElementById('vatgroup_1').checked==false){
				document.getElementById('tax').value=vat;
				document.getElementById('invtot').value=before_vat+vat;
			} else {
...
function pass_week_tar()
{
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		opener.document.form1.txtref.value  = XMLAddress1[0].childNodes[0].nodeValue;
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("dtfrom");	
		opener.document.form1.dtfrom.value = XMLAddress1[0].childNodes[0].nodeValue;
		
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("dtfr");	
		opener.document.form1.dtfr.value = XMLAddress1[0].childNodes[0].nodeValue;
