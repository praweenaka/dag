...
<div id="menus_wrapper">
    ...
    //                     	echo "<li><a href=\"stores.php\"><span><span>Stores</span></span></a></li>";
				...
				<div id="sec_menu">
					...
					$sql="select * from view_userpermission where username='".$_SESSION['UserName']."' and docname='Consumtion Update' and grp='System Utilities' and doc_view=1 and block='0'";
					//echo $sql;
					$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 	
					if ($row = mysqli_fetch_array($result)){			
					...
					$sql="select * from view_userpermission where username='".$_SESSION['UserName']."' and docname='Stock Adjustment' and grp='System Utilities' and doc_view=1 and block='0'";
					$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 	
					if ($row = mysqli_fetch_array($result)){			
						echo "<li><a href=\"stock_adjustment.php\" target=\"_blank\" class=\"sm1\" >Stock Adjustment</a></li>";
					}
					
					$sql="select * from view_userpermission where username='".$_SESSION['UserName']."' and docname='Commission Registry' and grp='System Utilities' and doc_view=1 and block='0'";
					$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 	
					if ($row = mysqli_fetch_array($result)){			
						echo "<li><a href=\"search_commission.php\" target=\"_blank\" class=\"sm1\" >Commission Registry</a></li>";
					}
					
					$sql="select * from view_userpermission where username='".$_SESSION['UserName']."' and docname='Unlock Invoice' and grp='System Utilities' and doc_view=1 and block='0'";
					$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 	
					if ($row = mysqli_fetch_array($result)){			
						echo "<li><a href=\"new\home.php?url=invoice_info\" target=\"_blank\" class=\"sm1\" >Unlock Invoice</a></li>";
					}
