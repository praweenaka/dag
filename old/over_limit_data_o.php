...
if ($_GET["Command"] == "view_from1") {
	...
	<tr>
	  ...
	  <td background=\"\" ><font color=\"#FFFFFF\">THT L3M</font></td>
	  <td background=\"\" ><font color=\"#FFFFFF\">THT N3M</font></td>
	...
	if ((strtolower($_SESSION["CURRENT_USER"]) == "rohan") or (strtolower($_SESSION["CURRENT_USER"]) == "rohan1")) {
		$sql = "Select REF_NO, C_CODE,NAME,SAL_EX,class,sum(GRAND_TOT) as amount from view_repord_brand_btr where INVNO = '0' and CANCELL = '0' AND  Forward = 'WD' and Result = 'P'  Group by  C_CODE,NAME,SAL_EX,class order by C_CODE";

	} else {
		if (strtolower($_SESSION["CURRENT_USER"]) == "md") {
			$sql = "Select REF_NO, C_CODE,NAME,SAL_EX,class,sum(GRAND_TOT) as amount from view_repord_brand_btr where INVNO = '0' and CANCELL = '0' AND  Forward = 'MD' and Result = 'P'  Group by  C_CODE,NAME,SAL_EX,class order by C_CODE";
			...
			if ((strtolower($_SESSION["CURRENT_USER"]) == "errol") or (strtolower($_SESSION["CURRENT_USER"]) == "errol1")) {
				...
				$sql = "Select REF_NO, C_CODE,NAME,SAL_EX,class,sum(GRAND_TOT) as amount from view_repord_brand_btr where INVNO = '0' and CANCELL = '0' AND Limit_need > '0' and Forward = 'MM' and Result='P' and approveby = '0'  Group by  C_CODE,NAME,SAL_EX,class order by C_CODE";

			} else {

				$sql = "Select REF_NO, C_CODE,NAME,SAL_EX,class,sum(GRAND_TOT) as amount from view_repord_brand_btr where INVNO = '0' and CANCELL = '0' AND Limit_need > '0' and Forward = 'MM' and Result='P'  Group by  C_CODE,NAME,SAL_EX,class order by C_CODE";
	...
	$i = 0;
	// limit
	$resultt = mysqli_query($GLOBALS['dbinv'], $sql);
	while ($row = mysqli_fetch_array($resultt)) {
		...
		//overlimit
		$overlimit = $limit - $out - $rtn_ch - $row["amount"];
		...
		$chq_re=0;
		$rs0 = "SELECT sum(che_amount) as reatot from s_invcheq where cus_code='" . $row["C_CODE"] . "'  and ( che_date>='" . $adddate . "')  and ( che_date<='" . $adddate1 . "') and trn_type = 'REC'";
		$result_rs0 = mysqli_query($GLOBALS['dbinv'], $rs0);
		while ($row_rs0 = mysqli_fetch_array($result_rs0)) {
			if (!is_null($row_rs0["reatot"])) {
		...
		$rs0 = "select sum(CA_CASSH) as ca_reatot from s_crec where CA_CODE = '" . $row["C_CODE"] . "' and ( CA_DATE>='" . $adddate . "')  and ( CA_DATE<='" . date("Y-m-d") . "') and CANCELL = '0' and flag='REC' ";
		$result_rs0 = mysqli_query($GLOBALS['dbinv'], $rs0);
		while ($row_rs0 = mysqli_fetch_array($result_rs0)) {
			if (!is_null($row_rs0["ca_reatot"])) {				 
		...
		$o90ret=0;
		$adddate = date('Y-m-d', strtotime("-90 days"));
		$rs0 = "SELECT sum(CR_CHEVAL) as out1 from s_cheq where  (CR_DATE>='" . $adddate . "')and (CR_DATE<='" . date("Y-m-d") . "') and CR_C_CODE = '" . $row["C_CODE"] . "' and CR_FLAG='0' ";
		$result_rs0 = mysqli_query($GLOBALS['dbinv'], $rs0);

		if ($row_rs0 = mysqli_fetch_array($result_rs0)) {
			if (!is_null($row_rs0["out1"])) { $o90ret = $row_rs0["out1"];
			}
		}
		...
		$Ch_pen=0;
		$rs0 = "SELECT sum(che_amount) as out1 from s_invcheq where ch_count_ret='0' and cus_code='" . $row["C_CODE"] . "'  and ( che_date>'" . date("Y-m-d") . "' or che_date ='" . date("Y-m-d") . "') ";
		$result_rs0 = mysqli_query($GLOBALS['dbinv'], $rs0); 
		if ($row_rs0 = mysqli_fetch_array($result_rs0)) {
		...
		$ord_no = $row['REF_NO'];
		$ResponseXML .= "<tr><td onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $row['C_CODE'] . "</div></td>
   						<td onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $row['NAME'] . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $limit . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $out . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $rtn_ch . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . 0 . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $row['amount'] . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $overlimit . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . 0 . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $retpd . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $o60 . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . 0 . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $last3 . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $o90ret . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . $n3m . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . 0 . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . 0 . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div>" . 0 . "</div></td>
						<td align=right ><div>". $row['class'] . "</div></td>
						<td onclick=\"appro('" . $i . "', '0');\"><div>---</div></td>
						<td ><input type=\"text\"></td>
						<td><div>". $row['SAL_EX'] . "</div></td></tr>";
	}
...
if ($_GET["Command"] == "view_from") {
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		if ($_SESSION['company'] == "THT") {
			...
			if ($row_rsven = mysqli_fetch_array($result_rsven)) {
				if ($row_rsven["commoncode"] != "0") {
					$rs0 = "Select sum(CR_CHEVAL-PAID) as out1 from s_cheq  where  CR_CHEVAL > PAID  and CR_C_CODE='" . trim($row_rsven["commoncode"]) . "' and CR_FLAG != 'c'  ";
		...
		if ((($myg[8] <= 0) or ($myg[11] != "0") or ($myg[5] > 0)) or (($myg[8] > 0) or ($myg[11] != "0") or ($myg[5] > 0))) {
			...
			$ResponseXML .= "<tr><td onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div id=\"" . $myg01 . "\">" . $myg[1] . "</div></td>
						...
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div align=right id=\"" . $myg03 . "\">" . number_format($myg[3], 0, ".", ",") . "</div></td>
                                                ...
                                                $ResponseXML .= "
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div align=right id=\"" . $myg06 . "\">" . number_format($myg[6], 0, ".", ",") . "</div></td>
						...
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div align=right id=\"" . $myg16 . "\">" . $myg[16] . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div align=right id=\"" . $myg17 . "\">" . $myg[17] . "</div></td>
						<td align=right onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $ord_no . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" ><div align=right id=\"" . $myg18 . "\">" . $myg[18] . "</div></td>
			...
			if (trim($_POST[$myg20]) == "YES") {
				...
				if (($_POST[$myg05] > 0) or ($_POST[$myg11] > 0)) {
					...
					if ($_POST[$myg08] < 0) {
						...
						$sql = "insert into tmpcrlmt (sdate, stime, username, tmpLmt, Class, Rep, cusCode, crLmt, cat, remark) values ('" . date("Y-m-d") . "','" . date("H:m:s") . "' ,'" . $_SESSION["CURRENT_USER"] . "'," . (($myg08_val - 10) * -1) . " ,'" . trim($_POST[$myg19]) . "','" . trim($_POST[$myg21]) . "','" . trim($_POST[$myg01]) . "'," . $_POST[$myg03] . ", 'C' , '" . trim($_POST[$myg22]) . "')";
						$result = mysqli_query($GLOBALS['dbinv'], $sql);
						//echo "3-". $sql;
