...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        function othersales() {
			...
			$strinv = "select stk_no from s_trn where ledindi = 'TRN' and sdate = '2015-04-04' group by stk_no";
		
			$resulti = mysqli_query($GLOBALS['dbinv'], $strinv);
            while ($row_RSINVO03 = mysqli_fetch_array($resulti)) {
			...
			$strinv .= " and stk_no ='" . $row_RSINVO03['stk_no'] . "'";
            ...
            while ($row_RSINVO01 = mysqli_fetch_array($result2)) {
                ...
                if ($yy == "y") {
                    ...
                    $insert[] = "('" . $row_RSINVO01['STK_NO'] . "', '" . $row_rsVENDOR["DESCRIPT"] . "',  '" . $cat . "', '" . ($mon1 - $Gmon1) . "', '" . ($mon2 - $Gmon2) . "', '" . ($mon3 - $Gmon3) . "' , '" . ($mon4 - $Gmon4) . "','" . ($mon5 - $Gmon5) . "','" . ($mon6 - $Gmon6) . "', '" . ($mon7 - $Gmon7) . "', '" . ($mon8 - $Gmon8) . "', '" . ($mon9 - $Gmon9) . "' , '" . ($mon10 - $Gmon10) . "','" . ($mon11 - $Gmon11) . "','" . ($mon12 - $Gmon12) . "','','" . $row_rsVENDOR['BRAND_NAME'] . "', '" . $_SESSION["CURRENT_USER"] . "','" . ($pmon1) . "', '" . ($pmon2) . "', '" . ($pmon3) . "' , '" . ($pmon4) . "','" . ($pmon5) . "','" . ($pmon6) . "', '" . ($pmon7) . "', '" . ($pmon8) . "', '" . ($pmon9) . "' , '" . ($pmon10) . "','" . ($pmon11) . "','" . ($pmon12) . "' ,  '" . $row_rsVENDOR["model"] . "', '" . $row_rsVENDOR["GROUP1"] . "','" . $row_rsVENDOR["GROUP2"] . "', '" . $row_rsVENDOR["PART_NO"] . "')";
                }
            }
			}
			
            $sql_RSMONSALES = "insert into 6moncons(Cus_Code, cus_name, cat,  month1, month2, month3,month4,month5,month6,month7,month8,month9,month10,month11,month12,sal_ex,brand, user_id, pmon1, pmon2, pmon3,pmon4,pmon5,pmon6,pmon7,pmon8,pmon9,pmon10,pmon11,pmon12,model,group1,group2,part_no) values " . implode(",", $insert);
		...
		<th  width='450'>Description</th><th >Part No</th>";
         ...
         $brand="";
             ...
             $sql_sql = "SELECT cus_code,cus_name,brand,part_no from 6moncons where cus_code <> '' and user_id='" . $_SESSION["CURRENT_USER"] . "' group by cus_code,cus_name,brand,part_no order by  brand,model,group1,group2";
            $result_sql = mysqli_query($GLOBALS['dbinv'], $sql_sql);

            while ($row_sql = mysqli_fetch_array($result_sql)) {
                 
				 	if ($brand != $row_sql["brand"]) {
					echo "
			<tr>
			<th colspan=3 align=left><b>" . $row_sql["brand"] . "</b></th>
			</tr>";
					$brand = $row_sql["brand"];
				}
