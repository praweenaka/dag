...
while ($rows = mysqli_fetch_array($result)){
	...
	while($row_s_mas = mysqli_fetch_array($result_s_mas)){
		...
		$result_ord =mysqli_query($GLOBALS['dbinv'],$sql_ord);
		if($row_ord = mysqli_fetch_array($result_ord)){
    		$sql_ordmas="select * from s_ordmas where REFNO='".$row_ord["REFNO"]."'";
			$result_ordmas =mysqli_query($GLOBALS['dbinv'],$sql_ordmas);
			if($row_ordmas = mysqli_fetch_array($result_ordmas)){
...
function all()
{
	...
	while($row_s_mas = mysqli_fetch_array($result_s_mas)){
		...
		if ($month==1){
		 ...
		 $sql1="Insert into tmpstkmve (STK_NO, BRAND_NAME, DESCRIPT, QTYINHAND, PART_NO,  shdate, ordqty, m1, m2,m3,m4,m5,m6,m7,m8,m9,m10,m11,m12 ) values('".$STK_NO."', '".$brand_name."', '".$descript."', ".$QTYINHAND.", '".$PART_NO."', '".$shdate."', ".$ordqty.", ".$m1.", ".$m2.", ".$m3.", ".$m4.", ".$m5.", ".$m6.", ".$m7.", ".$m8.", ".$m9.", ".$m10.", ".$m11.", ".$m12.")";
		$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
		if (!$result1) {
 ...
 echo "<table border=1><tr><td>Item No</td><td>Description</td><td>Part No</td>";
	$month = date("m",strtotime($mydate));
		if ($month==12){
			echo "<th>Jan</th>";
			echo "<th>Feb</th>";
	...
	while($row = mysqli_fetch_array($result)){
		...
		echo "<td>".$row["m11"]."</td>";
		echo "<td>".$row["m12"]."</td>";
		$mtot = ($row["m1"]+$row["m2"]+$row["m3"]+$row["m4"]+$row["m5"]+$row["m6"]+$row["m7"]+$row["m8"]+$row["m9"]+$row["m10"]+$row["m11"]+$row["m12"]);
		$mavg =$mtot /12;
		echo "<td>". number_format($mtot, 0, ".", ",")  ."</td>";
		echo "<td>". number_format($mavg, 0, ".", ",") ."</td>";
		echo "<td>". number_format($row["QTYINHAND"], 0, ".", ",") ."</td>";
		echo "<td>". 0 ."</td>";
		echo "</tr>";
	}
