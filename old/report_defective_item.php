...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        function summery2() {
            ...
            $rtxsumhead = "Tyre House Trading (Pvt) Ltd,. ";

            $txtdrange = " from " . " " . date("Y-m-d", strtotime($_GET["dtfrom"])) . " to " . date("Y-m-d", strtotime($_GET["dtto"]));
            $rtxdaterange = " from " . " " . date("Y-m-d", strtotime($_GET["dtfrom"])) . " to " . date("Y-m-d", strtotime($_GET["dtto"]));

            echo "<center>" . $row_invpara['COMPANY'] . "<br><BR>";
            echo "<center>" . "Complaint Report Summery " . $txtdrange . "<br><BR>";
...
//Dim rst2 As New ADODB.Recordset 'total sum of amount & qty for non recommended claims rejected
        ...
        function invRec() {
            ...
            if ($_GET["cmbbrand"] == "All") {
                if ($_GET["chkcus"] != "on") {
                    $sql = "SELECT * from viewdef where (SDATE='" . $_GET["dtfrom"] . "'or SDATE>'" . $_GET["dtfrom"] . "') and  (SDATE='" . $_GET["dtto"] . "'or SDATE<'" . $_GET["dtto"] . "') and CANCELL = '0' ";
                } else {
                    $sql = "SELECT * from viewdef where c_code= '" . trim($_GET["cuscode"]) . "' and (SDATE='" . $_GET["dtfrom"] . "'or SDATE>'" . $_GET["dtfrom"] . "') and  (SDATE='" . $_GET["dtto"] . "'or SDATE<'" . $_GET["dtto"] . "') and CANCELL = '0' ";
                }
            } else {

                if ($_GET["chkcus"] != "on") {
                    $sql = "SELECT * from viewdef where BRAND_NAME='" . $_GET["cmbbrand"] . "' and  (SDATE='" . $_GET["dtfrom"] . "'or SDATE>'" . $_GET["dtfrom"] . "') and  (SDATE='" . $_GET["dtto"] . "'or SDATE<'" . $_GET["dtto"] . "') and CANCELL = '0' ";
                } else {
                    $sql = "SELECT * from viewdef where BRAND_NAME='" . $_GET["cmbbrand"] . "' and c_code= '" . trim($_GET["cuscode"]) . "' and (SDATE='" . $_GET["dtfrom"] . "'or SDATE>'" . $_GET["dtfrom"] . "') and  (SDATE='" . $_GET["dtto"] . "'or SDATE<'" . $_GET["dtto"] . "') and CANCELL = '0' ";
                }
            }

            //echo $sql;
            if ($_GET['salesrep'] != "All") {
                $sql .= " and sal_ex = '" . $_GET['salesrep'] . "'";
            }
            $sql .= " order by STK_NO, c_code ";
            ...
            while ($row_rsPrInv = mysqli_fetch_array($result_rsPrInv)) {
                ...
                if ($STK_NO != $row_rsPrInv["STK_NO"]) {
                    if ($i != 0) {
                        ...
                        echo "<td>&nbsp;</td>";
                        echo "<td>&nbsp;</td>";
                        echo "<td align=right><b>" . number_format($AMOUNT1, 2, ".", ",") . "</b></td>";
                        echo "</tr>";
                ...
                echo "<td  align=left>" . $row_rsPrInv["Ref_per"] . "</td>";

                $amo = (($row_rsPrInv["AMOUNT"] - ($row_rsPrInv["AMOUNT"] * $row_rsPrInv["dis"] / 100)) * $row_rsPrInv["Ref_per"] / 100);
                $amo = $amo / (1 + $row_rsPrInv['vatrate'] / 100);
                echo "<td  align=right>" . number_format($amo, 2, ".", ",") . "</td>";
            ...
            echo "<td>&nbsp;</td>";
            echo "<td>&nbsp;</td>";
            echo "<td align=right><b>" . number_format($AMOUNTtot1, 2, ".", ",") . "</b></td>";
            echo "</tr>";
...
//Dim rst2 As New ADODB.Recordset 'total sum of amount & qty for non recommended claims rejected
      ...
      <tr>
        ...
        function defall() {
            ...
            if ($_GET["datewise"] == "on") {
                $sql_rsPrInv = "Select * from  view_clamas_brand where returndate >= '" . $_GET["dtfrom"] . "' and returndate <= '" . $_GET["dtto"] . "'";
            } else if ($_GET["datewise1"] == "on") {
                $sql_rsPrInv = "Select * from  view_clamas_brand where entdate >= '" . $_GET["dtfrom"] . "' and entdate <= '" . $_GET["dtto"] . "' and gatepass='0000-00-00'";
            } else {
                $sql_rsPrInv = "Select * from  view_clamas_brand where entdate >= '" . $_GET["dtfrom"] . "' and entdate <= '" . $_GET["dtto"] . "'";
            }
            if ($_GET["cmbbrand"] != "All") {
                $sql_rsPrInv .= " and brand = '" . trim($_GET["cmbbrand"]) . "'";
            }

            if (($_GET["chkcus"] == "on") and ( $_GET["cuscode"] != "")) {
                $sql_rsPrInv .= " and ag_code = '" . trim($_GET["cuscode"]) . "'";
            }


            if ($_GET["cmb_refund"] == "Recommended") {
                $sql_rsPrInv .= " and refund = 'Recommended'";
            } else {
                if ($_GET["cmb_notreco"] != "All") {

                    if ($_GET["cmb_notreco"] != "Allowed") {
                        $sql_rsPrInv .= " and refund = 'Not Recommended' and commercialy = '0'";
                        if ($_GET["cmb_approv"] != "All") {
                            $sql_rsPrInv .= " and approve_md_wd='" . $_GET["cmb_approv"] . "'";
                        }
                    } else {
                        $sql_rsPrInv .= " and refund = 'Not Recommended' and commercialy = '1'";
                        if ($_GET["cmb_approv"] != "All") {
                            $sql_rsPrInv .= " and approve_md_wd='" . $_GET["cmb_approv"] . "'";
                        }
                    }
            ...
            if ($_GET["stkno"] != "") {
                $sql_rsPrInv .= "  and stk_no='" . $_GET["stkno"] . "'";
            }
            if ($_GET["cmbtype"] != "All") {
                $sql_rsPrInv .= "  and class='" . $_GET["cmbtype"] . "'";
            }
            ...
            $q3 = 0;
            if ($_GET["datewise"] == "on") {
                $sql_rsPrInv1 = "Select sum(Amount) as amount, sum(qty) as qty,refund,commercialy  from  view_clamas_brand where returndate >= '" . $_GET["dtfrom"] . "' and returndate <= '" . $_GET["dtto"] . "'";
            } else if ($_GET["datewise1"] == "on") {
                $sql_rsPrInv1 = "Select sum(Amount) as amount, sum(qty) as qty,refund,commercialy  from  view_clamas_brand where entdate >= '" . $_GET["dtfrom"] . "' and entdate <= '" . $_GET["dtto"] . "' and gatepass='0000-00-00'";
            } else {
                $sql_rsPrInv1 = "Select sum(Amount) as amount, sum(qty) as qty,refund,commercialy  from  view_clamas_brand where entdate >= '" . $_GET["dtfrom"] . "' and entdate <= '" . $_GET["dtto"] . "'";
            }

            if ($_GET["cmbbrand"] != "All") {
                $sql_rsPrInv1 .= " and brand = '" . trim($_GET["cmbbrand"]) . "'";
            }

            if (($_GET["chkcus"] == "on") and ( $_GET["cuscode"] != "")) {
                $sql_rsPrInv1 .= " and ag_code = '" . trim($_GET["cuscode"]) . "'";
            }

            if ($_GET["cmb_refund"] == "Recommended") {
                $sql_rsPrInv1 .= " and refund = 'Recommended'";
            } else {
                if ($_GET["cmb_notreco"] != "All") {

                    if ($_GET["cmb_notreco"] != "Allowed") {
                        $sql_rsPrInv1 .= " and refund = 'Not Recommended' and commercialy = '0'";
                        if ($_GET["cmb_approv"] != "All") {
                            $sql_rsPrInv1 .= " and approve_md_wd='" . $_GET["cmb_approv"] . "'";
                        }
                    } else {
                        $sql_rsPrInv1 .= " and refund = 'Not Recommended' and commercialy = '0'";
                    }
            ...
            if ($_GET["stkno"] != "") {
                $sql_rsPrInv1 .= "  and stk_no='" . $_GET["stkno"] . "'";
            }
            if ($_GET["cmbtype"] != "All") {
                $sql_rsPrInv1 .= " and class='" . $_GET["cmbtype"] . "'";
            }

            $sql_rsPrInv1 .= " group by refund,commercialy";

            $result_rsPrInv1 = mysqli_query($GLOBALS['dbinv'], $sql_rsPrInv1);

            while ($row_rsPrInv1 = mysqli_fetch_array($result_rsPrInv1)) {

                if (($row_rsPrInv1["Refund"]) == "Recommended") {
                    $a1 = $a1 + $row_rsPrInv1["amount"];
                    $q1 = $q1 + $row_rsPrInv1["qty"];
                }
                if (($row_rsPrInv1["Refund"] == "Not Recommended") and $row_rsPrInv1["Commercialy"] > 0) {
                    $a2 = $a2 + $row_rsPrInv1["amount"];
                    $q2 = $q2 + $row_rsPrInv1["qty"];
                }
                if (($row_rsPrInv1["Refund"] == "Not Recommended") and $row_rsPrInv1["Commercialy"] == 0) {
                    $a3 = $a3 + $row_rsPrInv1["amount"];
                    $q3 = $q3 + $row_rsPrInv1["qty"];
                }
            }
            ...
            if ($_GET['radio'] != "Option7") {
                ...
                while ($row_rsPrInv = mysqli_fetch_array($result_rsPrInv)) {
                    ...
                    if ($ok == "1") {
                        ...
                        if ($mbrand != $row_rsPrInv["brand"]) {
                            echo "<tr>";
                            echo "<td colspan='16'><b>" . $row_rsPrInv["brand"] . "</b></td>";
                            echo "</tr>";
                            $Refund = "";
                        }
                        if ($Refund != $row_rsPrInv["Refund"]) {
                            echo "<tr align=center>";
                            echo "<td>&nbsp;</td><td align=left colspan=16><b>" . $row_rsPrInv["Refund"] . "</b></td>";
                            echo "</tr>";
                            $Refund = $row_rsPrInv["Refund"];
                        }
                ...
                echo "<td width=50><b>Gate Pass Date</b></td>";
                echo "<td width=50><b>Return Date</b></td>";
                echo "<td width=50><b>Inoice No</b></td>";
                echo "<td width=50><b>Amount</b></td>";
                echo "</tr>";
                ...
                while ($row_rsPrInv = mysqli_fetch_array($result_rsPrInv)) {
                    ...
                    if ($ok == "1") {
                        ...
                        if ($mbrand != $row_rsPrInv["brand"]) {
                            echo "<tr>";
                            echo "<td colspan='15'><b>" . $row_rsPrInv["brand"] . "</b></td>";
                            echo "</tr>";
                            $Refund = "";
                        }
                        if ($Refund != $row_rsPrInv["Refund"]) {
                            echo "<tr align=center>";
                            echo "<td>&nbsp;</td><td align=left colspan=15><b>" . $row_rsPrInv["Refund"] . "</b></td>";
                            echo "</tr>";
                            $Refund = $row_rsPrInv["Refund"];
                        }
                        ...
                        if ($row_rsPrInv["Refund"] == "Not Recommended") {
                            ...
                            if ($Commercialy != $row_rsPrInv["Commercialy"]) {
                                ...
                                echo "<td>&nbsp;</td>";
                                if ($row_rsPrInv["Commercialy"] == "0") {
                                    echo "<td align=left colspan=15 ><b>Not Allowed</b></td>";
                                } else {
                                    echo "<td align=left colspan=15 ><b>Allowed on Commercial Value</b></td>";
                                }
                        ...
                        echo "<td align=left>" . trim($row_rsPrInv["tc_ob"]) . " / " . trim($row_rsPrInv["Mn_ob"]) . "</td>";
                        echo "<td align=left>" . $row_rsPrInv["rem_per"] . "</td>";
                        echo "<td align=left>" . $row_rsPrInv["DGRN_NO"] . "  " . $row_rsPrInv["DGRN_NO2"] . "</td>";
                        echo "<td align=right>" . $row_rsPrInv["Amount"] . "</td>";
                        if ($row_rsPrInv["gatepass"] == "0000-00-00") {
                            echo "<td align=right></td>";
                        } else {
                        ...
                        echo "<td align=right>";
                        $strinv = "";
                        $sql = "select C_INVNO from s_ut where cre_no_no = '" . $row_rsPrInv["DGRN_NO"] . "' or cre_no_no = '" . $row_rsPrInv["DGRN_NO2"] . "' order by c_invno";
                        $result_rsPrInv_set = mysqli_query($GLOBALS['dbinv'], $sql);
                        while ($row_rsPrInv_Set = mysqli_fetch_array($result_rsPrInv_set)) {
                            $strinv .= $row_rsPrInv_Set["C_INVNO"] . " <br> ";
                        }
                        echo $strinv;
                        echo "</td>";
                        echo "<td align=right>";
                        $strinva = "";
                        $sql = "select C_PAYMENT from s_ut where cre_no_no = '" . $row_rsPrInv["DGRN_NO"] . "' or cre_no_no = '" . $row_rsPrInv["DGRN_NO2"] . "' order by c_invno";
                        $result_rsPrInv_set = mysqli_query($GLOBALS['dbinv'], $sql);
                        while ($row_rsPrInv_Set = mysqli_fetch_array($result_rsPrInv_set)) {
                            $strinva .= $row_rsPrInv_Set["C_PAYMENT"] . " <br> ";
                        }
                        echo $strinva;
                        echo "</td>";
                        echo "</tr>";
...
//Dim rst2 As New ADODB.Recordset 'total sum of amount & qty for non recommended claims rejected
            ...
            echo "<td width=50><b>CCA 3</b></td>";
            echo "<td width=50><b>OCV 3</b></td>";

            echo "<td width=50><b>Sold To Del.</b></td>";
            echo "<td width=50><b>Sold To Cus.</b></td>";
            ...
            while ($row_rsPrInv = mysqli_fetch_array($result_rsPrInv)) {
                ...
                if ($ok == "1") {
                    ...
                    echo "<td align=left>" . $row_rsPrInv["CCA3"] . "</td>";
                    echo "<td align=left>" . $row_rsPrInv["OCV3"] . "</td>";

                    echo "<td align=left>" . $row_rsPrInv["Sold_date"] . "</td>";
