...
if ($_GET["cmbdev"] == "Computer") { $dev = "1"; }
 ...
 if ($_GET["radio"]=="optdaly") { 
	...
	if ($_SESSION['company']=="BEN"){
		$sql = "SELECT * from c_bal where  SDATE='" . $_GET["dtfrom"] . "' and trn_type='REC' and DEV!='" . $dev . "' and CANCELL = '0' ";
 ...
 if ($_GET["radio"]=="optperiod") { 
 		...
 		$sql = "SELECT * from c_bal where  (SDATE='" . $_GET["dtfrom"] . "' or SDATE>'" . $_GET["dtfrom"] . "' ) and (SDATE='" . $_GET["dtto"] . "' or SDATE<'" . $_GET["dtto"] . "' )  and trn_type='REC'  and CANCELL = '0'";
...
if ($_GET['cmbtype'] !="All") {
	if ($_GET['cmbtype'] == "RTN") {
		$sql .= " and refno like 'RTC%'";
	} else {
		$sql .= " and refno not like 'RTC%'";
	}
