...
/////////// Sales Summery////////////////////////////////////////
	...
	function type_wise()
	{
        ...
        function repoall() {
            ...
            //$row = mysqli_fetch_array($result);
                ...
                while ($row_rst = mysqli_fetch_array($result_rst)) {
					...
					} else if(($_GET['DT1']>="2017-06-01") AND ($_SESSION["CURRENT_DEP"]=="Battery & Tube" OR  $_SESSION["CURRENT_DEP"]=="DISTRIBUTION")) { 
						$sql_rep .= " and ( RGROUP = 'Battery & Tube' or  RGROUP = 'DISTRIBUTION')";
					} else {
					...
					if(($_GET['DT1']>="2017-06-01") AND ($mgroup=="Battery & Tube" OR  $mgroup=="DISTRIBUTION")) {
						$mgroup	= "Battery & Tube";	
					}		
					
                    $insert[] = "('".trim($row_rep["REPCODE"])."', ".$SAL.", ".$ret.", '".$mname."', '".trim($mgroup)."',  ".$mnet.", ".$tar.", '".$_SESSION["CURRENT_USER"]."')";
                }
				}
            
			   if ($_GET["cmbbrand"] == "All") {
                    if ($_GET["chkdef"] != "on") {
                        $sql_rst2 = "SELECT sum(AMOUNT) as grand_tot,sum(AMOUNT/(1+vatrate/100)) as grand_tot1,SAL_EX FROM view_cbal_brand WHERE  (flag1 = '0')  AND trn_type!='REC' and trn_type!='DGRN' and trn_type!='ARN' AND trn_type!='INC' AND  trn_type!='PAY' AND (SDATE>'" . $_GET["DT1"] . "'or SDATE='" . $_GET["DT1"] . "')  AND( SDATE<'" . $_GET["DT2"] . "'or  SDATE='" . $_GET["DT2"] . "') and CANCELL='0' and DEV!='" . $dev . "'";
                    }

                    if ($_GET["chkdef"] == "on") {
                        $sql_rst2 = "SELECT sum(AMOUNT) as grand_tot,sum(AMOUNT/(1+vatrate/100)) as grand_tot1,SAL_EX FROM view_cbal_brand WHERE  (flag1 = '0')  AND trn_type!='REC' and trn_type!='ARN' AND trn_type!='INC' AND  trn_type!='PAY' AND (SDATE>'" . $_GET["DT1"] . "'or SDATE='" . $_GET["DT1"] . "')  AND( SDATE<'" . $_GET["DT2"] . "'or  SDATE='" . $_GET["DT2"] . "') and CANCELL='0' and DEV!='" . $dev . "' ";
                    }
                } else {

                    if ($_GET["chkdef"] != "on") {
                        $sql_rst2 = "SELECT sum(AMOUNT) as grand_tot,sum(AMOUNT/(1+vatrate/100)) as grand_tot1,SAL_EX FROM view_cbal_brand WHERE (flag1 = '0') and brand='" . $_GET["cmbbrand"] . "'  AND trn_type!='REC'and trn_type!='DGRN' and trn_type!='ARN' AND trn_type!='INC' AND  trn_type!='PAY' AND (SDATE>'" . $_GET["DT1"] . "'or SDATE='" . $_GET["DT1"] . "')  AND( SDATE<'" . $_GET["DT2"] . "'or  SDATE='" . $_GET["DT2"] . "') and CANCELL='0' and DEV!='" . $dev . "'";
                    }

                    if ($_GET["chkdef"] == "on") {
                        $sql_rst2 = "SELECT sum(AMOUNT) as grand_tot,sum(AMOUNT/(1+vatrate/100)) as grand_tot1,SAL_EX FROM view_cbal_brand WHERE (flag1 = '0') and brand='" . $_GET["cmbbrand"] . "'  AND trn_type!='REC' and trn_type!='ARN' AND trn_type!='INC' AND  trn_type!='PAY' AND (SDATE>'" . $_GET["DT1"] . "'or SDATE='" . $_GET["DT1"] . "')  AND( SDATE<'" . $_GET["DT2"] . "'or  SDATE='" . $_GET["DT2"] . "') and CANCELL='0' and DEV!='" . $dev . "'  ";
                    }
                }
				if ($_GET["salesrep"] != "All") {
			...
			$sql_rst2 .= " group by SAL_EX";
                ...
                while ($row_rst2 = mysqli_fetch_array($result_rst2)) {
					...
					$sql_t = "select sum(Target) as target,rep_code  from reptrn where rep_code='" . trim($row_rep["REPCODE"]) . "' group by rep_code";    
					if ($_GET["cmbbrand"] !="All") {
						$sql_t .= " and brand = '" . $_GET["cmbbrand"] . "'";
					}
             	    $result_tar = mysqli_query($GLOBALS['dbinv'], $sql_t) ; 
            	    ...
            	    while ($row_tar = mysqli_fetch_array($result_tar)) {
             		   if ($row_tar["target"] > 0) {
						 ...
						 $sql_rep = "SELECT * FROM s_salrep where REPCODE ='"  . $row_tar["rep_code"] . "'";
                    $result_rep = mysqli_query($GLOBALS['dbinv'], $sql_rep) ; 
                    if ($row_rep = mysqli_fetch_array($result_rep)) {
                 	$mname = trim($row_rep["Name"]);
                    ...
                    $insert[] = "('".trim($row_rep["REPCODE"])."', ".$SAL.", ".$ret.", '".$mname."', '".trim($row_rep["RGROUP"])."',  ".$mnet.", ".$tar.", '".$_SESSION["CURRENT_USER"]."')";
					}
					   }
		}
			
					$sql_tem = "insert into tmprepsale(rep, grossale, grossgrn, Name, rgroup, net, targ, user_id) values ". implode($insert, ",")  ;
            $result_tem =mysqli_query($GLOBALS['dbinv'], $sql_tem) ;   
		...
		<th>Balance</th>
			...
			$sql = "select rep,Name,rgroup,targ,sum(grossale) as grossale,sum(grossgrn) as grossgrn,sum(net) as net from tmprepsale where user_id='".$_SESSION["CURRENT_USER"]."' group by  rep,name,rgroup,targ order by rgroup,rep desc";
            $result = mysqli_query($GLOBALS['dbinv'],$sql) ; 
		...
		<th>GRN %</th>
            ...
            $sql = "select rep,Name,rgroup,sum(grossale) as grossale,sum(grossgrn) as grossgrn,sum(net) as net from tmprepsale where user_id='".$_SESSION["CURRENT_USER"]."' group by  rep,name,rgroup order by rgroup,rep desc";
            $result = mysqli_query($GLOBALS['dbinv'], $sql) ; 
            while ($row = mysqli_fetch_array($result)) {
                if ($row["rgroup"] != $rgroup) {
                    if ($grossale != 0) {
