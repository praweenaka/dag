...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		if ($_GET["cmbbrand"] == "All") {
			$sql_smas = "select * from viewsubmas1 where STO_CODE='" . $_GET["com_dep"] . "' AND  QTY1> 0  limit 400";
		}
		if ($_GET["cmbbrand"] != "All") {
		...
		while ($row_smas = mysqli_fetch_array($result_smas)) {
			...
			while ($balqty != 0) {
				...
				while ($row_artrn = mysqli_fetch_array($result_artrn)) {
					...
					$sql = "select * from s_ginmas where ar_no='" . trim($row_artrn["REFNO"]) . "' and  DEP_TO = '" . $_GET["com_dep"] . "' limit 25";
					...
					if ($rowc <> 0) {
						...
						while ($row_artrn1 = mysqli_fetch_array($result_artrn1)) {
							...
							$sql = "select * from s_trn where refno='" . trim($row_artrn1["REF_NO"]) . "' and stk_no = '" . trim($row_artrn1["STK_NO"]) . "' and  DEP_TO = '" . $_GET["com_dep"] . "' limit 25";
							$result_artrn2 = mysqli_query($GLOBALS['dbinv'], $sql);
							$rowc1 = mysqli_num_rows($result_artrn2);
	...
	if (!$res) {
		...
		<center>
			...
			<table width="1200" border="1">
				<tr>
					<th>Stock No</th><th width="250">Description</th><th>Part No</th><th>Qty In Ha</th><th>No of Days</th><th>Un Sold Stock</th><th>L/C No</td> <th>AR Date</th><th>AR No</th><th>AR Qty</th><th>Total Value</th><th>Cost Value</th><th>Unsold Value</th>
				</tr>
				<?php
				$brand = "";
				$STK_NO = "";
