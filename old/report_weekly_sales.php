...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        function reports_monthly() {
            ...
            $sql_salma = "select sum(GRAND_TOT/(1 +GST/100)) AS GRAND_TOT , month(sdate1) as cmonth, SAL_EX, Brand, c_code from s_salma where Accname != 'NON STOCK'  and year(sdate1)='" . $year . "' and CANCELL='0' AND DEV!='" . $GLOBALS["sysdiv"] . "'   ";

            if ($_GET["cmbcat"] != "All") {
                $sql_salma .= " and sal_ex = '" . $_GET["cmbcat"] . "'";
            }
            $sql_salma .= "  group by month(sdate1), SAL_EX,Brand,c_code";
            //echo $sql_salma;
            $result_salma = mysqli_query($GLOBALS['dbinv'],$sql_salma);
            $i=0;
            $insert="";
            while ($row_salma = mysqli_fetch_array($result_salma)) {
                //echo date("d",strtotime($row_salma["sdate1"]))."-".$row_salma["GRAND_TOT"]." / ";
                $mon1 = 0;
                $mon2 = 0;
                $mon3 = 0;
                $mon4 = 0;
            ...
            if ($_GET["chkdef"] == "on") {
                $sql_cbal = "select sum(AMOUNT/(1 +vatrate/100)) as AMOUNT,month(sdate1) as cmonth,SAL_EX,brand,CUSCODE from c_bal where  year(sdate1)='" . $year . "'   and trn_type!='ARN' and trn_type!='REC' and trn_type!='INC' and trn_type!='PAY' AND DEV!='" . $GLOBALS["sysdiv"] . "' and flag1 != '1' ";
            }
            if ($_GET["chkdef"] != "on") {
                $sql_cbal = "select sum(AMOUNT/(1 +vatrate/100)) as AMOUNT,month(sdate1) as cmonth,SAL_EX,brand,CUSCODE  from c_bal where year(sdate1)='" . $year . "' and trn_type!='ARN' and trn_type!='REC'and trn_type!='DGRN' and trn_type!='INC' and trn_type!='PAY' AND DEV!='" . $GLOBALS["sysdiv"] . "' and flag1 != '1'  ";
            }
//echo $sql_cbal;
            if ($_GET["cmbcat"] != "All") {
            ...
            $sql_cbal .= " group by month(sdate1), SAL_EX, brand, CUSCODE";
            //echo $sql_cbal;
        ...
        function reports() {
            ...
            $sql_salma = "select sum(GRAND_TOT/(1 +GST/100)) AS GRAND_TOT , sdate1,SAL_EX,Brand,c_code from s_salma where Accname != 'NON STOCK'  and month(sdate1)='" . $mon . "' and year(sdate1)='" . $year . "'  and CANCELL='0' AND DEV!='" . $GLOBALS["sysdiv"] . "'   ";

            if ($_GET["cmbcat"] != "All") {
                $sql_salma .= " and sal_ex = '" . $_GET["cmbcat"] . "'";
            }
            $sql_salma .= "  group by sdate1,SAL_EX,Brand,c_code";
            ...
            while ($row_salma = mysqli_fetch_array($result_salma)) {
                ...
                if (date("d", strtotime($row_salma["sdate1"])) < 8) {
                    $wk1 = $row_salma["GRAND_TOT"];
                } else if (date("d", strtotime($row_salma["sdate1"])) < 15) {
                    $wk2 = $row_salma["GRAND_TOT"];
                } else if (date("d", strtotime($row_salma["sdate1"])) < 22) {
                    $wk3 = $row_salma["GRAND_TOT"];
                } else if (date("d", strtotime($row_salma["sdate1"])) < 29) {
...
$i = 0;
            //echo $wk1."/".$wk2."/".$wk3."/".$wk4."/".$wk5." - ";
            if ($_GET["chkdef"] == "on") {
                $sql_cbal = "select sum(AMOUNT/(1 +vatrate/100)) as AMOUNT,sdate1,SAL_EX,brand,cuscode from c_bal where  month(sdate1)='" . $mon . "' and year(sdate1)='" . $year . "'   and trn_type!='ARN' and trn_type!='REC' and trn_type!='INC' and trn_type!='PAY' AND DEV!='" . $GLOBALS["sysdiv"] . "' and flag1 != '1' ";
            }
            if ($_GET["chkdef"] != "on") {
                $sql_cbal = "select sum(AMOUNT/(1 +vatrate/100)) as AMOUNT,sdate1,SAL_EX,brand,cuscode  from c_bal where  month(sdate1)='" . $mon . "' and year(sdate1)='" . $year . "' and trn_type!='ARN' and trn_type!='REC'and trn_type!='DGRN' and trn_type!='INC' and trn_type!='PAY' AND DEV!='" . $GLOBALS["sysdiv"] . "' and flag1 != '1'  ";
            }
            ...
            $sql_cbal .= " group by sdate1,SAL_EX,brand,cuscode";
            $result_cbal = mysqli_query($GLOBALS['dbinv'],$sql_cbal);
            while ($row_cbal = mysqli_fetch_array($result_cbal)) {
                $wk1 = 0;
                $wk2 = 0;
                ...
                if (date("d", strtotime($row_cbal["sdate1"])) < 8) {
                    $wk1 = $row_cbal["AMOUNT"] * -1;
                } else if (date("d", strtotime($row_cbal["sdate1"])) < 15) {
                    $wk2 = $row_cbal["AMOUNT"] * -1;
                } else if (date("d", strtotime($row_cbal["sdate1"])) < 22) {
                    $wk3 = $row_cbal["AMOUNT"] * -1;
                } else if (date("d", strtotime($row_cbal["sdate1"])) < 29) {
