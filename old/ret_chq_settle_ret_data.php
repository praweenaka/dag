...
if($_GET["Command"]=="save_crec")
{
	...
	$sql1="insert into s_crec (CA_REFNO, CA_DATE, CA_CODE, CA_CASSH, CA_AMOUNT, CA_SALESEX, DEPARTMENT, DEV, FLAG, pay_type) values ('".trim($_GET["recno"])."', '".$_GET["invdate"]."', '".$_GET["cuscode"]."',  ".$_GET["cashtot"].", ".$_GET["txtpaytot"].", '".$_GET["salesrep"]."', 'O', '".$_SESSION['dev']."', 'RET', '".$_GET["paytype"]."')";
	//echo $sql1;
	$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
 ...
 // if (($_GET["paytype"]=="J/Entry") or ($_GET["paytype"]=="Cash TT") or ($_GET["paytype"]=="R/Deposit")){	
	...
	$sql="insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, l_year) values ('".trim($_GET["recno"])."', '".$_GET["invdate"]."', '". trim($_GET["accno2"])."', ".$_GET["txtpaytot"].", 'REC', 'CRE', '".$l_lmem."', '0', 'R', 0, '".$_SESSION['company']."', '', '0', ".date("Y").")";
	//echo $sql;
	$result=mysqli_query($GLOBALS['dbinv'],$sql, $dbacc);
	
	
	$sql="insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, l_lmem, l_flag2, l_flag3, l_yearfl, comcode, chno, recdate, l_year) values ('".trim($_GET["recno"])."', '".$_GET["invdate"]."', '". trim($_GET["accno"])."', ".$_GET["txtpaytot"].", 'REC', 'DEB', '".$l_lmem."', '0', 'R', 0, '".$_SESSION['company']."', '', '0', ".date("Y").")";
	//echo $sql;
	$result=mysqli_query($GLOBALS['dbinv'],$sql, $dbacc);
...
//  }	
	...
	if ($_GET["txtoverpay"]>0){
		$sql1="Insert into c_bal (REFNO, SDATE, trn_type, CUSCODE, AMOUNT, BALANCE, SAL_EX, DEV) values ('".trim($_GET["recno"])."', '".$_GET["invdate"]."', 'REC',  '".$_GET["cuscode"]."', ".$_GET["txtoverpay"].", ".$_GET["txtoverpay"].", '".$_GET["salesrep"]."', '".$_SESSION['dev']."')";
		$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
