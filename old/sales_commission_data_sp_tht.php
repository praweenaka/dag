...
if ($_GET["Command"] == "grnhistory") {
    ...
    while ($row_rsgen = mysqli_fetch_array($result_rsgen)) {
        ...
        //$sql_rs_salm= "Select sdate1, GRAND_TOT, DUMMY_VAL from s_salma where Accname <> 'NON STOCK' and REF_NO='" . $row_rsgen["INVOICENO"] . "' ";
        $sql_rs_salm = "Select sdate1, GRAND_TOT, DUMMY_VAL from s_salma where REF_NO='" . $row_rsgen["INVOICENO"] . "' ";
    ...
    $sql_rsgen = "select * from cred where CANCELL='0' and month(sdate1) =" . $month . " and   year(sdate1) =" . $year . " and C_SALEX='" . trim($_GET["cmbrep"]) . "' and DEV='" . $_GET["cmbdev"] . "' ";
    $result_rsgen = mysqli_query($GLOBALS['dbinv'], $sql_rsgen);
    while ($row_rsgen = mysqli_fetch_array($result_rsgen)) {
        ...
        if ($row_rsbal = mysqli_fetch_array($result_rsbal)) {
            ...
            //$sql_rs_salm= "Select sdate1,GRAND_TOT,DUMMY_VAL from s_salma where Accname <> 'NON STOCK' and REF_NO='" . $row_rsgen["C_INVNO"] . "'";
            $sql_rs_salm = "Select sdate1,GRAND_TOT,DUMMY_VAL from s_salma where REF_NO='" . $row_rsgen["C_INVNO"] . "'";
            $result_rs_salm = mysqli_query($GLOBALS['dbinv'], $sql_rs_salm);
            if ($row_rs_salm = mysqli_fetch_array($result_rs_salm)) {
...
if ($_POST["Command"] == "savegrn") {

    $r = 1;
    while ($_POST["grngrid"] > $r) {
        ...
        if ($_POST[$Commi] == "") {
            ...
            $Commi_val = $_POST[$Commi];
        }

        if ($_POST[$commman] == "") {
            $commman_val = 0;
        } else {
            $commman_val = $_POST[$commman];
        }

        if ($_POST[$gtype] == "GRN") {

            $sql_inv = "update s_crnma set DUMMY_VAL=" . $Commi_val . " ,DIS1=" . $commman_val . "  where  REF_NO='" . $_POST[$grnno] . "'";
            //echo $sql_inv;
            $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
        } else {

            $sql_inv = "update cred set dummy_val=" . $Commi_val . " ,SETTLED=" . $commman_val . "  where  C_REFNO='" . $_POST[$grnno] . "'";
            //echo $sql_inv;
            $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
        }
...
if ($_GET["Command"] == "view_report") {
    ...
    $sql_inv = "select * from s_salma where SAL_EX='" . $rep . "' and month(sdate1)=" . $month . " AND YEAR(sdate1)=" . $year . " and CANCELL='0' and DEV='" . $_GET["cmbdev"] . "'";
//    echo $sql_inv;
    $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
...
//echo $txtbalSAleTOT.'/'.$txtt2.'/'.$txtt1;
//10974918.66/10000000.00/8000000.00
        ...
        if ($row_sttr = mysqli_fetch_array($result_sttr)) {
            ...
            while ($row_sttr = mysqli_fetch_array($result_sttr)) {
                ...
                if ($apdays < $day1) {
                    ...
                    if ($row_inv["DEV"] == "0") {
                        $commission = $cat1 * $row_sttr["ST_PAID"] * 0.01 / (1 + ($row_inv['GST'] / 100));
                    }
                }
                ...
                if (($apdays > $day1 - 1) and ( $apdays < $day2)) {
                    if ($cat2 > 0) {
                        ...
                        if ($row_inv["DEV"] == "0") {
                            $commission = $cat2 * $row_sttr["ST_PAID"] * 0.01 / (1 + ($row_inv['GST'] / 100));
                        }
                    } else {
                        //echo "(S3)";
            ...
            $due = $row_inv["GRAND_TOT"] - $row_inv["TOTPAY"];
            $sql = "insert into tmpcommition(SDATE, REFNO, COS_CODE, cus_name, AMOUNT, DUE, brand, dev, PAY_AMOUNT)  values ('" . $row_inv["sdate1"] . "', '" . trim($row_inv["REF_NO"]) . "', '" . $row_inv["C_CODE"] . "', '" . $row_inv["CUS_NAME"] . "', " . $row_inv["GRAND_TOT"] . ", " . $due . ", '" . $row_inv["Brand"] . "', '" . $row_inv["dev"] . "', 0)";
            //echo "2-".$sql;
            $result = mysqli_query($GLOBALS['dbinv'], $sql);
        }
    ...
    if ($_GET["cmbdev"] == "0") {
        ...
        while ($row_CRN = mysqli_fetch_array($result_CRN)) {
            ...
            $sql_invdiv = "select *  from s_salma where REF_NO='" . $row_CRN["C_INVNO"] . "'";
...
//            echo "3-".$sql;
        ...
        while ($row_grn = mysqli_fetch_array($result_grn)) {
            ...
            $sql_invdiv = "select *  from s_salma where REF_NO='" . $row_grn["INVOICENO"] . "'";
            $result_invdiv = mysqli_query($GLOBALS['dbinv'], $sql_invdiv);
...
if ($_GET["Command"] == "com_lock") {
    ...
    if (($txtComBal + $txtAdd) > 0) {
        ...
        $returnchk = $txtRetChkAmou_Do + $txtRetChkAmou_D1 + $txtret;
...
if ($_GET["Command"] == "lock_advance") {
    ...
    if ($row_commadva = mysqli_fetch_array($result_commadva)) {
        ...
        $txtover601 = str_replace(",", "", $_GET["txtover60"]); 
        $txtretch = str_replace(",", "", $_GET["txtretcheq"]); 
        $sql = "Update s_commadva set Over60out = '" .$txtover601 . "' where refno = '" . $mrefno . "' AND FLAG='ADV'";
        $result = mysqli_query($GLOBALS['dbinv'], $sql);

        $sql = "Update s_commadva set Returnchk = '" . $txtretch . "' where refno = '" . $mrefno . "' AND FLAG='ADV'";
        $result = mysqli_query($GLOBALS['dbinv'], $sql);
...
if ($_GET["Command"] == "view_summery") {
    ...
    $year = substr($_GET["dtMonth"], 0, 4);
    $month = substr($_GET["dtMonth"], 5, 2);
    $monthcomremark = substr($_GET["dtMonth"], 0, 7);

    $dtlas = $year . "-" . $month . "-01";
    $dtlas = date('Y-m-t', strtotime($dtlas));
    $caldays = " - 1 months";
    $dtlas = date('Y-m-d', strtotime($dtlas . $caldays));
    $dtlas = date('Y-m-t', strtotime($dtlas));
    $sql_commadva = "select sum(grand_tot-totpay) as out1 from s_salma where sdate1 <='" . trim($dtlas) . "' and sal_ex= '" . $_GET["cmbrep"] . "'and cancell = '0'  ";
    //echo $sql_commadva;
    $result_commadva = mysqli_query($GLOBALS['dbinv'], $sql_commadva);
    if ($row_commadva = mysqli_fetch_array($result_commadva)) {
        $ResponseXML .= "<out_tot><![CDATA[" . number_format($row_commadva["out1"], 2, ".", ",") . "]]></out_tot>";
    } else {
        $ResponseXML .= "<out_tot><![CDATA[0]]></out_tot>";
    }

    $sql_utmas123 = "SELECT * FROM commisionremark WHERE month  ='" . $monthcomremark . "' and rep='" . $_GET["cmbrep"] . "' and type='Bal'";

    $result_utmas123 = mysqli_query($GLOBALS['dbinv'], $sql_utmas123);
    if ($row_utmas123 = mysqli_fetch_array($result_utmas123)) {

        $ResponseXML .= "<TXTREMARK><![CDATA[" . $row_utmas123['remark'] . "]]></TXTREMARK>";
    } else {
    ...
    //$mrefno = trim($month) . $_GET["cmbrep"];
    //echo $mrefno;
    $sql_commadva = "select * from s_commadva where month(comdate)='" . trim($month) . "' and year(comdate)='" . trim($year) . "' and rep = '" . $_GET["cmbrep"] . "'and flag = 'BAL'";
    //echo $sql_commadva;
    $result_commadva = mysqli_query($GLOBALS['dbinv'], $sql_commadva);
    if ($row_commadva = mysqli_fetch_array($result_commadva)) {
        $ResponseXML .= "<txtdedamt2><![CDATA[" . $row_commadva["advance"] . "]]></txtdedamt2>";
        $ResponseXML .= "<txtper><![CDATA[" . $row_commadva["per"] . "]]></txtper>";
        $ResponseXML .= "<txtded><![CDATA[" . $row_commadva["ded"] . "]]></txtded>";
        $ResponseXML .= "<txtdedremark><![CDATA[" . $row_commadva["remark"] . "]]></txtdedremark>";
        $ResponseXML .= "<Txtadva><![CDATA[" . $row_commadva["advance"] . "]]></Txtadva>";
        $ResponseXML .= "<TXTADJ><![CDATA[" . $row_commadva["adj"] . "]]></TXTADJ>";
    }

    //$GRAND_TOT=0;
    //$outstanding=0;
    //$sql="select * from s_salma where Accname != 'NON STOCK' and month(sdate1)='".$month."' and year(sdate1)='".$year."' and SAL_EX='".$_GET["cmbrep"]."' and CANCELL='0'";
    $sql = "select * from s_salma where month(sdate1)='" . $month . "' and year(sdate1)='" . $year . "' and SAL_EX='" . $_GET["cmbrep"] . "' and CANCELL='0'";
    // echo $sql."Ashan";
    $result = mysqli_query($GLOBALS['dbinv'], $sql);
    while ($row = mysqli_fetch_array($result)) {

        $usr[] = "('" . $row["REF_NO"] . "', '" . $row["TRN_TYPE"] . "', '" . $row["sdate1"] . "', '" . $row["C_CODE"] . "', '" . $row["CUS_NAME"] . "', '" . $row["C_ADD1"] . "', '" . $row["TYPE"] . "', '" . $row["SAL_EX"] . "', " . $row["DISCOU"] . ", " . $row["AMOUNT"] . ", " . $row["GST"] . ", " . $row["GRAND_TOT"] . ", " . $row["DUMMY_VAL"] . ", " . $row["DIS"] . ", " . $row["DIS1"] . ", " . $row["DIS2"] . ", " . $row["DIS_RUP"] . ", " . $row["CASH"] . ", " . $row["TOTPAY"] . ", '" . $row["ORD_NO"] . "', '" . $row["ORD_DA"] . "', '" . $row["SETTLED"] . "', '" . $row["TOTPAY1"] . "', '" . $row["DES_CAT"] . "', '" . $row["DEPARTMENT"] . "', '" . $row["REMARK"] . "', '" . $row["CANCELL"] . "', '" . $row["BTT"] . "', '" . $row["VAT"] . "', " . $row["VAT_VAL"] . ", '" . $row["Brand"] . "', '" . $row["DEV"] . "', '" . $row["Account"] . "', '" . $row["Accname"] . "', '" . $row["Costcenter"] . "', " . $row["RET_AMO"] . ", " . $row["cre_pe"] . ", '" . $row["Comm"] . "', '" . $row["red"] . "', '" . $row["deli_date"] . "', '" . $row["seri_no"] . "', '" . $row["points"] . "', '" . $row["LOCK1"] . "', '" . $row["deliin"] . "', " . $row["SVAT"] . ", '" . $row["REQ_DATE"] . "', '" . $row["tmp_no"] . "', '" . $row["vat_no"] . "', '" . $row["s_vat_no"] . "', '" . $_SESSION['UserName'] . "')";
        ...
        while ($row_sttr = mysqli_fetch_array($result_sttr)) {
            ...
            if (trim($row_sttr["ST_FLAG"]) == "UT") {
                if ((!is_null($row['deli_date'])) and ( $row['deli_date'] != "0000-00-00")) {
                    ...
                    $diff = abs(strtotime($row_sttr["ST_DATE"]) - strtotime($row["sdate1"]));
                    $days = floor($diff / (60 * 60 * 24));
                }
            } else {
                if ((!is_null($row['deli_date'])) and ( $row['deli_date'] != "0000-00-00")) {
                    ...
                    $diff = abs(strtotime($row_sttr["st_chdate"]) - strtotime($row["sdate1"]));
                    $days = floor($diff / (60 * 60 * 24));
                }
            }
    ...
    while ($row_st = mysqli_fetch_array($result_st)) {
        ...
        if (trim($row_st["ST_FLAG"]) == "UT") {
            if ((is_null($row["deli_date"]) == false) and ( $row["deli_date"] != "0000-00-00")) {
                ...
                $diff = abs(strtotime($row_st["ST_DATE"]) - strtotime($row["sdate1"]));
                $days = floor($diff / (60 * 60 * 24));
            }
        } else {
            if ((is_null($row["deli_date"]) == false) and ( $row["deli_date"] != "0000-00-00")) {
                ...
                $diff = abs(strtotime($row_st["st_chdate"]) - strtotime($row["sdate1"]));
                $days = floor($diff / (60 * 60 * 24));
            }
        }
    ...
    $m_update = true;
    if ($m_update == true) {
        $sql1 = "update s_salma set DIS='0' where month(sdate1)='" . $month . "' AND year(sdate1)='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0'";
        $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
    ...
    $sql1 = "SELECT SUM(AMOUNT)AS RETAMU,SUM(AMOUNT/(1+vatrate/100))AS RETAMU1  FROM c_bal WHERE (flag1 = '0') and MONTH(sdate1)='" . $month . "' AND YEAR(sdate1) ='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0' and (trn_type='GRN' or  trn_type='CNT' ) and DEV='" . $_GET["cmbdev"] . "' ";
    //echo $sql1;
    //Ashan
    $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
    ...
    if ($_GET["cmbdev"] == "All") {
        $sql_inv = "select SUM(GRAND_TOT) AS SALEAMU,SUM(GRAND_TOT/(1+gst/100)) AS SALEAMU1  from tmp_s_salma where month(SDATE)='" . $month . "' AND YEAR(SDATE)='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0' and userid='" . $_SESSION['UserName'] . "'";
        $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
        ...
        //ashan
        $sql_inv = "select SUM(GRAND_TOT) AS SALEAMU,SUM(GRAND_TOT/(1+gst/100)) AS SALEAMU1  from tmp_s_salma where month(SDATE)='" . $month . "' AND YEAR(SDATE)='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0' and dev='" . trim($_GET["cmbdev"]) . "' and userid='" . $_SESSION['UserName'] . "'";
        $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
        if ($row_inv = mysqli_fetch_array($result_inv)) {
            if (is_null($row_inv["SALEAMU"]) == false) {
                $TOTSALE = $row_inv["SALEAMU"];
    ...
    //ashan
    //$sql_inv="select SUM(GRAND_TOT) AS SALEAMU  from tmp_s_salma where Accname != 'NON STOCK' and month(SDATE)='".$month."' AND YEAR(SDATE)='".$year."' AND SAL_EX='".$_GET["cmbrep"]."' AND CANCELL='0' and userid='".$_SESSION['UserName']."'";
    $sql_inv = "select SUM(GRAND_TOT) AS SALEAMU,SUM(GRAND_TOT/(1+gst/100)) AS SALEAMU1   from tmp_s_salma where month(SDATE)='" . $month . "' AND YEAR(SDATE)='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0' and userid='" . $_SESSION['UserName'] . "'";
    $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
    if ($row_inv = mysqli_fetch_array($result_inv)) {
        if (is_null($row_inv["SALEAMU"]) == false) {
    ...
    if ($_GET["cmbdev"] == "1") {
        $txtBalsale = $TOTSALE_w - $_GET["txtRetChkAmou_D1"];
        //$ResponseXML .= "<txtBalsale><![CDATA[".$txtBalsale."]]></txtBalsale>";
    }

    if ($_GET["cmbdev"] == "0") {
        $txtBalsale = ($TOTSALE_w - $totret_w) - $_GET["txtRetChkAmou_Do"];
        //$ResponseXML .= "<txtBalsale><![CDATA[".$txtBalsale."]]></txtBalsale>";
    }
    ...
    $sql_inv = "select * from tmp_s_salma where month(SDATE)='" . $month . "' AND YEAR(SDATE)='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0' and userid='" . $_SESSION['UserName'] . "' order by C_CODE";
    //echo $sql_inv;
    $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
    while ($row_inv = mysqli_fetch_array($result_inv)) {
        $invamou = 0;
        ...
        if ($row_inv["DEV"] == "1") {
            ...
            if ($row_sttr = mysqli_fetch_array($result_sttr)) {
                if (is_null($row_sttr["INVPAID"]) == false) {
                    ...
                    if ($cat1 > 0) {
                        ...
                        $TOTcOMMpAY_D1_w = $TOTcOMMpAY_D1_w + ($row_sttr["INVPAID"] / (1 + $row_inv['GST'] / 100));
                        ...
                        $TOTnOcOMMpAY_D1_w = $TOTnOcOMMpAY_D1_w + ($row_sttr["INVPAID"] / (1 + $row_inv['GST'] / 100));
                        ...
                        if ($_GET["cmbdev"] == "1") {
                            ...
                            $sql = "select Brand from tmp_s_salma where REF_NO='" . trim($row_inv["REF_NO"]) . "' and userid='" . $_SESSION['UserName'] . "'";
                            $result = mysqli_query($GLOBALS['dbinv'], $sql);
            ...
            if ($row_sttr = mysqli_fetch_array($result_sttr)) {
                $tot_Comm_cat_1_D1 = $tot_Comm_cat_1_D1 + $row_sttr["INVPAID"];
                $tot_Comm_cat_1_D1_w = $tot_Comm_cat_1_D1_w + ($row_sttr["INVPAID"] / (1 + $row_inv['GST'] / 100));
                // echo "Cat1-".$cat1;
                if ($cat1 > 0) {
                    //echo "----ok";
                    $TOTcOMMpAY_D1 = $TOTcOMMpAY_D1 + $row_sttr["INVPAID"];
            ...
            if ($row_sttr = mysqli_fetch_array($result_sttr)) {
                ...
                if (is_null($row_sttr["INVPAID"]) == false) {
                    ...
                    if ($cat1 > 0) {
                        ...
                        $TOTcOMMpAY_Do_w = $TOTcOMMpAY_Do_w + ($row_sttr["INVPAID"] / (1 + $row_inv['GST'] / 100));
                        ...
                        $TOTnOcOMMpAY_Do_w = $TOTnOcOMMpAY_Do_w + ($row_sttr["INVPAID"] / (1 + $row_inv['GST'] / 100));
...
//        echo '1';
//        echo $TOTSALE."//1111///";
//        echo $TOTcOMMpAY_D1."//22222222222///";
//        echo $TOTcOMMpAY_Do."//3333333///";
//        
//        
//        echo $TOTnOcOMMpAY_D1."//44444///";
//        echo $TOTnOcOMMpAY_Do."//555///";


        $txtout = $TOTSALE - $TOTcOMMpAY_Do - $TOTnOcOMMpAY_Do;
//        echo $TOTSALE.'/'.$TOTcOMMpAY_Do.'/'.$TOTnOcOMMpAY_Do; 
        $ResponseXML .= "<txtout><![CDATA[" . number_format($txtout, 2, ".", ",") . "]]></txtout>";
    ...
    if ($_GET["cmbdev"] == "All") {
        ...
        $txtNocomm = $TOTnOcOMMpAY_D1_w + $TOTnOcOMMpAY_Do_w;
        $ResponseXML .= "<txtNocomm><![CDATA[" . number_format($txtNocomm, 2, ".", ",") . "]]></txtNocomm>";
    ...
    $netsaleall = ($TOTSALEALL_w - $totret_w);
    $txtTotnet = ($TOTSALEALL_w - $totret_w);
    $ResponseXML .= "<txtTotnet><![CDATA[" . number_format($netsaleall, 2, ".", ",") . "]]></txtTotnet>";

    //====================Dis chng done coz req of malaka & Milroy================

    $txtTOTNocom = ($TOTnOcOMMpAY_D1_w + $TOTnOcOMMpAY_Do_w);
    $ResponseXML .= "<txtTOTNocom><![CDATA[" . number_format($txtTOTNocom, 2, ".", ",") . "]]></txtTOTNocom>";

    $txtpre = (($TOTnOcOMMpAY_D1_w + $TOTnOcOMMpAY_Do_w) + $_GET["txtRetChkAmou_D1"] + $_GET["txtRetChkAmou_Do"]) / $netsaleall * 100;
...
if ($_GET["Command"] == "calculation") {
    ...
    $sql_inv = "select * from s_salma where month(sdate1)='" . $month . "' AND YEAR(sdate1)='" . $year . "' AND SAL_EX='" . trim($_GET["cmbrep"]) . "' AND CANCELL='0' ";
            ...
            if ($row_sttr = mysqli_fetch_array($result_sttr)) {
                if (is_null($row_sttr["INVPAID"]) == false) {
                    if ($row_inv["DEV"] == "1") {
                        ...
                        $ComAmou = $ComAmou + $row_sttr["INVPAID"] * $cat1 * 0.01 / (1 + ($row_inv['GST'] / 100));
                        $cat1Comm = $cat1Comm + $row_sttr["INVPAID"] * $cat1 * 0.01 / (1 + ($row_inv['GST'] / 100));
                    }
                }
            }
            ...
            if ($row_sttr = mysqli_fetch_array($result_sttr)) {
                if (is_null($row_sttr["INVPAID"]) == false) {
                    if ($row_inv["DEV"] == "1") {
                        ...
                        $cat1SpComm = $cat1SpComm + $row_sttr["INVPAID"] * $cat1 * 0.01 / (1 + ($row_inv['GST'] / 100));
                    }
                }
            }
            ...
            if ($row_sttr = mysqli_fetch_array($result_sttr)) {
                if (is_null($row_sttr["INVPAID"]) == false) {
                    if ($row_inv["DEV"] == "1") {
                        ...
                        $t = $t + $row_sttr["INVPAID"];
                        $ComAmou = $ComAmou + $row_sttr["INVPAID"] * $cat2 * 0.01 / (1 + ($row_inv['GST'] / 100));
                        $cat2Comm = $cat2Comm + $row_sttr["INVPAID"] * $cat2 * 0.01 / (1 + ($row_inv['GST'] / 100));
                    }
                }
            }
            ...
            if ($row_sttr = mysqli_fetch_array($result_sttr)) {
                if (is_null($row_sttr["INVPAID"]) == false) {
                    if ($row_inv["DEV"] == "1") {
                        ...
                        $ComAmou = $ComAmou + $row_sttr["INVPAID"] * $cat3 * 0.01 / (1 + ($row_inv['GST'] / 100));
                        if (($cat3 == 0) and ( $_GET["txtpre"] <= 15)) {
                            $cat2NoComm = $cat2NoComm + $row_sttr["INVPAID"] * $cat2 * 0.01 / (1 + ($row_inv['GST'] / 100));
                            $ComAmou = $ComAmou + $row_sttr["INVPAID"] * $cat2 * 0.01 / (1 + ($row_inv['GST'] / 100));
                            if ($m_update == true) {
                                $row_inv["DIS"] = $row_inv["DIS"] + $row_sttr["INVPAID"] * $cat2 * 0.01 / (1 + ($row_inv['GST'] / 100));
                            }
                        }
                    }
                }
    ...
    while ($row_rsgen = mysqli_fetch_array($result_rsgen)) {
        ...
        if (is_null($row_rsgen["DIS1"]) == false) {
            $retcommamou = $retcommamou + $row_rsgen["DIS1"];
        }
    }
...
if ($_POST["Command"] == "save_advance") {
    ...
    while ($_POST["mgridcount"] > $i) {
        ...
        $sql_rsbrand = "Select * from brand_mas where barnd_name = '" . trim($_POST[$msgrid1]) . "' ";
        //echo $sql_rsbrand;
        ...
        if ($row_rsbrand = mysqli_fetch_array($result_rsbrand)) {
            ...
            if ($row_rsbrand["class"] == "TYRE") {
                $mtyre = $mtyre + str_replace(",", "", $_POST[$msgrid5]);
            }
            if ($row_rsbrand["class"] == "BATTERY") {
                $mbattery = $mbattery + str_replace(",", "", $_POST[$msgrid5]);
            }
            if ($row_rsbrand["class"] == "ALLOY WHEEL") {
                $malloy = $malloy + str_replace(",", "", $_POST[$msgrid5]);
            }
            if ($row_rsbrand["class"] == "TUBE") {
                $mtube = $mtube + str_replace(",", "", $_POST[$msgrid5]);
            }

            if ($row_rsbrand["class"] == "TYRE") {
                $mtyre_com = $mtyre_com + str_replace(",", "", $_POST[$msgrid6]);
            }
            if ($row_rsbrand["class"] == "BATTERY") {
                $mbattery_com = $mbattery_com + str_replace(",", "", $_POST[$msgrid6]);
            }
            if ($row_rsbrand["class"] == "ALLOY WHEEL") {
                $malloy_com = $malloy_com + str_replace(",", "", $_POST[$msgrid6]);
            }
            if ($row_rsbrand["class"] == "TUBE") {
                $mtube_com = $mtube_com + str_replace(",", "", $_POST[$msgrid6]);
            }
        }
    ...
    $mtyre_com = $mtyre_com * 50 / 100;
    $mbattery_com = $mbattery_com * 50 / 100;
    $malloy_com = $malloy_com * 50 / 100;
    $mtube_com = $mtube_com * 50 / 100;

    $year = date("Y", strtotime($_POST["dtMonth"]));
    $month = date("m", strtotime($_POST["dtMonth"]));

    $txtded = str_replace(",", "", $_POST["txtded"]);
    $txtad = str_replace(",", "", $_POST["txtad"]);
    $TXTADJ = str_replace(",", "", $_POST["TXTADJ"]);
    if ($TXTADJ == "") {
        $TXTADJ = 0;
    }
    $txtded1 = str_replace(",", "", $_POST["txtded1"]);
    $txtded2 = str_replace(",", "", $_POST["txtded2"]);
    $txtded3 = str_replace(",", "", $_POST["txtded3"]);
    $txtded4 = str_replace(",", "", $_POST["txtded4"]);
    $txtded5 = str_replace(",", "", $_POST["txtded5"]);
    $txtded6 = str_replace(",", "", $_POST["txtded6"]);
    $txtded7 = str_replace(",", "", $_POST["txtded7"]);
    $txtded8 = str_replace(",", "", $_POST["txtded8"]);

    $txtdedamou1 = str_replace(",", "", $_POST["txtdedamou1"]);
    $txtdedamou2 = str_replace(",", "", $_POST["txtdedamou2"]);
    $txtdedamou3 = str_replace(",", "", $_POST["txtdedamou3"]);
    $txtdedamou4 = str_replace(",", "", $_POST["txtdedamou4"]);
    $txtdedamou5 = str_replace(",", "", $_POST["txtdedamou5"]);
    $txtdedamou6 = str_replace(",", "", $_POST["txtdedamou6"]);
    $txtdedamou7 = str_replace(",", "", $_POST["txtdedamou7"]);
    $txtdedamou8 = str_replace(",", "", $_POST["txtdedamou8"]);

    $txtover60 = str_replace(",", "", $_POST["txtover60"]);
    $txtretioded = str_replace(",", "", $_POST["txtretioded"]);

    $txtdedremark = str_replace(",", "", $_POST["txtdedremark"]);
    $txtretcheq = str_replace(",", "", $_POST["txtretcheq"]);

    $mrefno = $month . "/" . $year . "-" . $_POST["cmbrep"] . "-" . $_POST["cmbdev"];
...
if ($_GET["Command"] == "advance_proces") {
    ...
    $sql_rs = "Select  sum(GRAND_TOT) as sales,sum(GRAND_TOT/(1+GST/100)) as sales1 from s_salma where CANCELL='0' and SAL_EX='" . $mrep . "' and year(sdate1)='" . $year . "' and month(sdate1)='" . $month . "'   ";
    $result_rs = mysqli_query($GLOBALS['dbinv'], $sql_rs);
    if ($row_rs = mysqli_fetch_array($result_rs)) {
        if (is_null($row_rs["sales"]) == false) {
            $txtsales = $row_rs["sales"];
            $txtsales_w = $row_rs["sales1"];
        }
    }

    $ResponseXML .= "<txtsales><![CDATA[" . number_format($txtsales, 2, ".", ",") . "]]></txtsales>";

    $sql_rs = "Select  sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where  CANCELL='0' and trn_type='CNT' and SAL_EX='" . $mrep . "' and year(sdate1)='" . $year . "' and month(sdate1)='" . $month . "' and flag1='0'   ";
    $result_rs = mysqli_query($GLOBALS['dbinv'], $sql_rs);
    if ($row_rs = mysqli_fetch_array($result_rs)) {
        if (is_null($row_rs["salesret"]) == false) {
            $txtcrn = $row_rs["salesret"];
            $txtcrn_w = $row_rs["salesret1"];
        }
    }

    $ResponseXML .= "<txtcrn><![CDATA[" . number_format($txtcrn, 2, ".", ",") . "]]></txtcrn>";

    $sql_rs = "Select  sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where CANCELL='0' and trn_type='GRN' and SAL_EX='" . $mrep . "' and year(sdate1)='" . $year . "' and month(sdate1)='" . $month . "' and flag1='0'  ";
    $result_rs = mysqli_query($GLOBALS['dbinv'], $sql_rs);
    if ($row_rs = mysqli_fetch_array($result_rs)) {
        if (is_null($row_rs["salesret"]) == false) {
            $txtrtn = $row_rs["salesret"];
            $txtrtn_w = $row_rs["salesret1"];
        }
    }
    ...
    $txtnett = ($txtsales_w - ($txtcrn_w + $txtrtn_w));
    ...
    $ResponseXML .= "<txtdedremark><![CDATA[" . ($txtdedremark) . "]]></txtdedremark>";
    ...
    if ($txtover60 <= 0) {
        ...
        $sql_rs_salm = "Select  * from s_salma where CANCELL='0' and  GRAND_TOT > TOTPAY and SAL_EX='" . $mrep . "'  ";
        $result_rs_salm = mysqli_query($GLOBALS['dbinv'], $sql_rs_salm);
        while ($row_rs_salm = mysqli_fetch_array($result_rs_salm)) {
            ...
            // $date2 = $row_rs_salm["sdate1"];
               // praweenaka 20.07.14
            if (is_null($row_rs_salm["deli_date"])) { 
                $date2 = $row_rs_salm["sdate1"];
            }else{
                $date2 = $row_rs_salm["deli_date"];
            }
// ===============================
    ...
    if ($txtnett != "") {
        if ($txtnett != 0) {
            $TXTRATO = ($txtover60 + $TXTADJ + $txtretcheq) / $txtnett * 100;
        }
    }


    $netSale = ($txtsales_w - $txtrtn_w - $txtcrn_w);
    ...
    $sql_rs_salm = "Select Brand , sum(GRAND_TOT) as sales,sum(GRAND_TOT/(1+GST/100)) as sales1 from s_salma where CANCELL='0' and DEV='" . $mdev . "' and SAL_EX='" . $mrep . "' and year(sdate1)='" . $year . "' and month(sdate1)='" . $month . "'  group by Brand  ";
    $result_rs_salm = mysqli_query($GLOBALS['dbinv'], $sql_rs_salm);
    while ($row_rs_salm = mysqli_fetch_array($result_rs_salm)) {
    ...
    $sql_rs_salm = "Select  brand, sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where CANCELL='0' and  trn_type='CNT' and SAL_EX='" . $mrep . "' and year(sdate1)='" . $year . "' and month(sdate1)='" . $month . "' and flag1='0' and DEV='" . $mdev . "' group by brand  ";
    ...
    $sql_rs_salm = "Select  brand, sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where DEV='" . $mdev . "' and trn_type='GRN' and SAL_EX='" . $mrep . "' and year(sdate1)='" . $year . "' and month(sdate1)='" . $month . "'  group by brand  ";
    $result_rs_salm = mysqli_query($GLOBALS['dbinv'], $sql_rs_salm);
    while ($row_rs_salm = mysqli_fetch_array($result_rs_salm)) {
    ...
    if ($_GET["chkinv"] == "on") {
        ...
        while ($GLOBALS[$Gridinv[$r][1]] != "") {
            if (($GLOBALS[$Gridinv[$r][6]] > 0) and ( $GLOBALS[$Gridinv[$r][1]] != "") and ( $GLOBALS[$Gridinv[$r][2]] != "") and ( $GLOBALS[$Gridinv[$r][5]] > 0)) {
                ...
                $sql_rsinv = "select Brand from s_salma where REF_NO='" . trim($GLOBALS[$Gridinv[$r][1]]) . "'";
