...
if ($_GET["Command"] == "grnhistory") {
    ...
    while ($row_rsgen = mysqli_fetch_array($result_rsgen)) {
        ...
        $sql_rs_salm = "Select SDATE, GRAND_TOT, DUMMY_VAL from s_salma where REF_NO='" . $row_rsgen["INVOICENO"] . "' ";
    ...
    while ($row_rsgen = mysqli_fetch_array($result_rsgen)) {
        ...
        if ($row_rsbal = mysqli_fetch_array($result_rsbal)) {
            ...
            $sql_rs_salm = "Select SDATE,GRAND_TOT,DUMMY_VAL from s_salma where REF_NO='" . $row_rsgen["C_INVNO"] . "'";
            $result_rs_salm = mysqli_query($GLOBALS['dbinv'], $sql_rs_salm);
            if ($row_rs_salm = mysqli_fetch_array($result_rs_salm)) {
...
if ($_GET["Command"] == "view_report") {
    ...
    $sql_inv = "select * from s_salma where SAL_EX='" . $rep . "' and month(SDATE)=" . $month . " AND YEAR(SDATE)=" . $year . " and CANCELL='0' and DEV='" . $_GET["cmbdev"] . "'";
    //echo $sql_inv;
    $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
...
if ($_GET["Command"] == "view_summery") {
    ...
    $dtlas = date('Y-m-t', strtotime($dtlas));
    $sql_commadva = "select sum(grand_tot-totpay) as out1 from s_salma where sdate <='" . trim($dtlas) . "' and sal_ex= '" . $_GET["cmbrep"] . "'and cancell = '0'  ";
    //echo $sql_commadva;
    $result_commadva = mysqli_query($GLOBALS['dbinv'], $sql_commadva);
    if ($row_commadva = mysqli_fetch_array($result_commadva)) {
    ...
    $sql = "select * from s_salma where month(SDATE)='" . $month . "' and year(SDATE)='" . $year . "' and SAL_EX='" . $_GET["cmbrep"] . "' and CANCELL='0'";
    // echo $sql."Ashan";
    $result = mysqli_query($GLOBALS['dbinv'], $sql);
...
if ($_GET["Command"] == "calculation") {
    ...
    $sql_inv = "select * from s_salma where month(SDATE)='" . $month . "' AND YEAR(SDATE)='" . $year . "' AND SAL_EX='" . trim($_GET["cmbrep"]) . "' AND CANCELL='0' ";
...
if ($_POST["Command"] == "save_advance") {
    ...
    $mtyre_com = $mtyre_com * 60 / 100;
    $mbattery_com = $mbattery_com * 60 / 100;
    $malloy_com = $malloy_com * 60 / 100;
    $mtube_com = $mtube_com * 60 / 100;
...
if ($_GET["Command"] == "advance_proces") {
    ...
    $sql_rs = "Select  sum(GRAND_TOT) as sales,sum(GRAND_TOT/(1+GST/100)) as sales1 from s_salma where CANCELL='0' and SAL_EX='" . $mrep . "' and year(SDATE)='" . $year . "' and month(SDATE)='" . $month . "'   ";
    $result_rs = mysqli_query($GLOBALS['dbinv'], $sql_rs);
    if ($row_rs = mysqli_fetch_array($result_rs)) {
    ...
    $sql_rs = "Select  sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where  CANCELL='0' and trn_type='CNT' and SAL_EX='" . $mrep . "' and year(SDATE)='" . $year . "' and month(SDATE)='" . $month . "' and flag1='0'   ";
    $result_rs = mysqli_query($GLOBALS['dbinv'], $sql_rs);
    if ($row_rs = mysqli_fetch_array($result_rs)) {
    ...
    $sql_rs = "Select  sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where CANCELL='0' and trn_type='GRN' and SAL_EX='" . $mrep . "' and year(SDATE)='" . $year . "' and month(SDATE)='" . $month . "' and flag1='0'  ";
    $result_rs = mysqli_query($GLOBALS['dbinv'], $sql_rs);
    if ($row_rs = mysqli_fetch_array($result_rs)) {
        if (is_null($row_rs["salesret"]) == false) {
    ...
    $sql_rs_salm = "Select Brand , sum(GRAND_TOT) as sales,sum(GRAND_TOT/(1+GST/100)) as sales1 from s_salma where CANCELL='0' and DEV='" . $mdev . "' and SAL_EX='" . $mrep . "' and year(SDATE)='" . $year . "' and month(SDATE)='" . $month . "'  group by Brand  ";
    $result_rs_salm = mysqli_query($GLOBALS['dbinv'], $sql_rs_salm);
    while ($row_rs_salm = mysqli_fetch_array($result_rs_salm)) {
    ...
    $sql_rs_salm = "Select  brand, sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where CANCELL='0' and  trn_type='CNT' and SAL_EX='" . $mrep . "' and year(SDATE)='" . $year . "' and month(SDATE)='" . $month . "' and flag1='0' and DEV='" . $mdev . "' group by brand  ";
    ...
    $sql_rs_salm = "Select  brand, sum(AMOUNT) as salesret,sum(AMOUNT/(1+vatrate/100)) as salesret1 from c_bal  where DEV='" . $mdev . "' and trn_type='GRN' and SAL_EX='" . $mrep . "' and year(SDATE)='" . $year . "' and month(SDATE)='" . $month . "'  group by brand  ";
    $result_rs_salm = mysqli_query($GLOBALS['dbinv'], $sql_rs_salm);
    while ($row_rs_salm = mysqli_fetch_array($result_rs_salm)) {
