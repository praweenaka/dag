...
if($_GET["Command"]=="view_summery")
{
	...
	if ($_GET["cmbdev"]=="All"){
		...
		$sql_inv="select SUM(GRAND_TOT) AS SALEAMU  from tmp_s_salma where Accname != 'NON STOCK' and month(SDATE)='".$month."' AND YEAR(SDATE)='".$year."' AND SAL_EX='".$_GET["cmbrep"]."' AND CANCELL='0' and dev='".trim($_GET["cmbdev"])."' and userid='".$_SESSION['UserName']."'";
		$result_inv=mysqli_query($GLOBALS['dbinv'],$sql_inv); 
		if ($row_inv = mysqli_fetch_array($result_inv)){
			if (is_null($row_inv["SALEAMU"])==false){
				$TOTSALE = $row_inv["SALEAMU"];
