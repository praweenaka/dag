...
if ($_GET["Command"] == "getdATA") {
    ...
    if ($_SESSION ["dev"] == "0") {
        $sql_ven = "select SDATE,REF_NO,C_CODE,CUS_NAME,Brand,GRAND_TOT,TOTPAY,cre_pe from view_salma_vendor where SAL_EX='" . trim($mcmbrep) . "' and month(SDATE) = '" . $Mon . "' and year(SDATE) = '" . $Yer . "' and CANCELL = '0' and DEV='0' order by REF_NO";
    } else if ($_SESSION["dev"] == "1") {
        $sql_ven = "select SDATE,REF_NO,C_CODE,CUS_NAME,Brand,GRAND_TOT,TOTPAY,cre_pe from view_salma_vendor where SAL_EX='" . trim($mcmbrep) . "' and month(SDATE) = '" . $Mon . "' and year(SDATE) = '" . $Yer . "' and CANCELL = '0' order by REF_NO";
...
if ($_GET["Command"] == "calcc") {
    ...
    $sql_inv = "select SUM(GRAND_TOT) AS SALEAMU,SUM(GRAND_TOT/(1 +gst/100)) AS SALEAMU1,sum(grand_tot-totpay) as outs,sum(totpay) as totpay from s_salma where month(SDATE)='" . $month . "' AND YEAR(SDATE)='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0' ";
    //echo $sql_inv;
    $result_inv = mysqli_query($GLOBALS['dbinv'], $sql_inv);
    if ($row_inv = mysqli_fetch_array($result_inv)) {
        if (is_null($row_inv["SALEAMU"]) == false) {
    ...
    $sql1 = "SELECT SUM(AMOUNT)AS RETAMU,SUM(AMOUNT/(1 +vatrate/100)) AS RETAMU1   FROM c_bal WHERE (flag1 = '0') and MONTH(SDATE)='" . $month . "' AND YEAR(SDATE) ='" . $year . "' AND SAL_EX='" . $_GET["cmbrep"] . "' AND CANCELL='0' and (trn_type='GRN' or  trn_type='CNT' )  ";
    $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
    if ($row1 = mysqli_fetch_array($result1)) {
    ...
    if ($_SESSION["dev"] == "0") {
        $sql_ven = "select SDATE,REF_NO,C_CODE,CUS_NAME,Brand,GRAND_TOT,TOTPAY,cre_pe from view_salma_vendor where SAL_EX='" . trim($_GET["cmbrep"]) . "' and month(SDATE) = '" . $month . "' and year(SDATE) = '" . $year . "' and CANCELL = '0' and DEV='0' order by REF_NO";
    } else if ($_SESSION["dev"] == "1") {
        $sql_ven = "select SDATE,REF_NO,C_CODE,CUS_NAME,Brand,GRAND_TOT,TOTPAY,cre_pe from view_salma_vendor where SAL_EX='" . trim($_GET["cmbrep"]) . "' and month(SDATE) = '" . $month . "' and year(SDATE) = '" . $year . "' and CANCELL = '0' order by REF_NO";
    }
