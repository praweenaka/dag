...
function load_calader(tar){
 ...
 $sql="select * from s_cusordmas where REF_NO='".$_GET["txtrefno"]."' ";
 $result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
...
<fieldset>
	...
	<table  width="40%" border="1" >
		<tr>
			...
			<th width='20%'>Order Value</th>
		</tr>
		
		<?php
		$sql = "select * from s_cusordmas where c_code = '" . $row['C_CODE']   . "' and sal_ex = '" . $row['SAL_EX'] ."' and result ='P' and invno='0' and forward='WD'";
		$result_p =mysqli_query($GLOBALS['dbinv'],$sql) ; 
 		while ($row_p = mysqli_fetch_array($result_p)) {
 			echo "<tr>";
			echo "<td  onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $row_p['REF_NO'] . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" >" . $row_p['REF_NO']   . "</td>";
			echo "<td  onClick=\"NewWindow('sales_ord_new_display.php?txtrefno=" . $row_p['REF_NO'] . "','mywin','900','700','yes','center');return false\" onFocus=\"this.blur()\" >" . $row_p['GRAND_TOT']   . "</td>";
			echo "</tr>";
 		}
		...
		?>
                                                	...
                                                	<legend>
  <div class="text_forheader">Order Details</div>
                                               	 </legend>             
...
<form name="form1" id="form1"> <input type="hidden" name="cmd_new" id="cmd_new" value="1" />
  ...
  <tr>
    ...
    <td width="10%"><input type="text" name="salesord1" id="salesord1" value="<?php echo $_GET["txtrefno"]; ?>" class="text_purchase3" onkeypress="keyset('searchcust',event);"  disabled="disabled"  />     </td>
    <td width="10%">
    ...
    <td width="10%"><input type="text" size="20" name="invdate" id="invdate" value="<?php echo $row["SDATE"]; ?>"  onfocus="load_calader('invdate');" class="text_purchase3" disabled="disabled"/></td>
    <td width="10%">&nbsp;</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3"><input type="text"  class="text_purchase1" name="firstname_hidden" id="firstname_hidden"  disabled="disabled"  value="<?php echo $row["C_CODE"]; ?>"/>
      <input type="text" class="text_purchase2" id="firstname" name="firstname" value="<?php echo $row["CUS_NAME"]; ?>" disabled="disabled" />
  ...
  <tr>
    ...
    <td colspan="2"><input type="text"  class="text_purchase4" value="<?php echo $row["C_ADD1"]; ?>"  id="cus_address" name="cus_address" /></td>
    <td>&nbsp;</td>
    <td><input type="text"  class="label_purchase" value="Department" disabled/></td>
    <?php
		$sql_st="select * from s_stomas where CODE='".$row["DEPARTMENT"]."'";
		$result_st =mysqli_query($GLOBALS['dbinv'],$sql_st);
		$row_st = mysqli_fetch_array($result_st);
	
	?>
    <td><input type="text"  class="text_purchase3" value="<?php echo $row_st["DESCRIPTION"]; ?>" id="cus_address" name="cus_address" /></td>
    <td><input type="text"  class="label_purchase" value="Brand" disabled="disabled"/></td>
   
    <td><input type="text"  class="text_purchase3" value="<?php echo $row["Brand"]; ?>" id="cus_address" name="cus_address" /> </td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    ...
    <td bgcolor="#00CCCC"><label>
   	<?php
	 if ($row["VAT"]=="1"){
      	echo "<input type=\"radio\" name=\"vatgroup\" value=\"vat\" id=\"vatgroup_0\"  checked=\"checked\" />";
	 } else {
    ...
    <?php
	 if ($row["VAT"]=="0"){
      	echo "<input type=\"radio\" name=\"vatgroup\" value=\"non\" id=\"vatgroup_0\"  checked=\"checked\" />";
	 } else {
	 	echo "<input type=\"radio\" name=\"vatgroup\" value=\"non\" id=\"vatgroup_0\"  />";
	 }
	?> 
  ...
  <tr>
    ...
    <?php
	$sql_rep="select * from s_salrep where REPCODE='".$row["SAL_EX"]."'";
	$result_rep =mysqli_query($GLOBALS['dbinv'],$sql_rep);
	$row_rep = mysqli_fetch_array($result_rep);
	
	?>
    <td><input type="text"  class="text_purchase3" value="<?php echo $row_rep["Name"]; ?>" id="cus_address" name="cus_address" />  </td>
    <td><input type="text"  class="label_purchase" value="Discount 1" disabled="disabled"/></td>
    <td><input type="text" size="5" name="discount_org1" id="discount_org1" value="<?php echo $row["DIS"]; ?>" onkeypress="keyset('itemd_hidden',event);" onblur="calc1_table_discount1();" class="text_purchase"/></td>
...
<table>
         		...
         		<?php
                    ...
                    $sql1="select * from s_cusordtrn where REF_NO='".$_GET["txtrefno"]."'";
 					$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
 					while($row1 = mysqli_fetch_array($result1)){
                                         
                            echo "<tr>
                              		<td width=\"10%\"  >".$row1["STK_NO"]."</font></td>
                              		<td width=\"40%\" >".$row1["DESCRIPT"]."</font></td>
                              		<td width=\"10%\" >".$row1["PRICE"]."</font></td>
                              		<td width=\"10%\" >".$row1["QTY"]."</font></td>
                                    <td width=\"10%\" >".$row1["DIS_per"]."</font></td>
                                    <td width=\"10%\" >".number_format((($row1["PRICE"]*$row1["QTY"])*(100-$row1["DIS_per"])/100), 2, ".", ",")."</font></td>
                           		</tr>";
					}			
                     ?>                                               
                   												</table>   </div>                                                 		</td>
  ...
  <tr>
	...
	?>
    <td><input type="text" size="15" name="tax" id="tax" value="<?php echo number_format($vat_val, 2, ".", ","); ?>"  class="text_purchase3"  disabled="disabled" /></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
