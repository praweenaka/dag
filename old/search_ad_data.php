		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
						...
						if ($_GET["Option1"]=="true"){
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
								/*if ($_GET["stname"]=="ord"){
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_admas where REF_NO like  '$letters%' limit 50") or die(mysqli_error());
								} else {*/
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_admas where REF_NO like  '$letters%' and  INVNO='0'  and CANCELL='0'  order by SDATE desc limit 50") or die(mysqli_error());
								//}	
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								/*if ($_GET["stname"]=="ord"){
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_admas where CUS_NAME like  '$letters%' limit 50 ") or die(mysqli_error()) or die(mysqli_error());
								} else {*/
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_admas where CUS_NAME like  '$letters%' and  INVNO='0'  and CANCELL='0'  order by SDATE desc limit 50") or die(mysqli_error()) or die(mysqli_error());
								//}
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
...
if ($_GET["Command"]=="pass_invno_cus_ord"){
	...
	// Call setcus
		...
		$sql_rsinv = mysqli_query($GLOBALS['dbinv'],"SELECT * FROM s_adtrn where REF_NO='" . $_GET["invno"] . "'") or die(mysqli_error());	
...
if ($_GET["Command"]=="pass_invno"){
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			$ResponseXML .= "<stname><![CDATA[".$_GET["stname"]."]]></stname>";
				...
				$sql = mysqli_query($GLOBALS['dbinv'],"Select * from s_admas where REF_NO='".$inv."'") or die(mysqli_error());
			...
			$invno=trim("CRI/ ").substr($tmpinvno, $lenth-7);*/
				...
				$sql_data = mysqli_query($GLOBALS['dbinv'],"delete from tmp_ad_data where tmp_no='".$_SESSION["tmp_no_ord"]."'") or die(mysqli_error());
				//echo "Select * from s_adtrn where REFNO='".$inv."'";
				$sql_data = mysqli_query($GLOBALS['dbinv'],"Select * from s_adtrn where REF_NO='".$inv."'") or die(mysqli_error());
				while($row = mysqli_fetch_array($sql_data)){
					...
					$sql_tmp = mysqli_query($GLOBALS['dbinv'],"Insert into tmp_ad_data(str_invno, str_code, str_description, cur_rate, cur_qty, dis_per,cur_discount, cur_subtot, brand, tmp_no) values ( '".$inv."', '".$row['STK_NO']."', '".$row['DESCRIPT']."', ".$row['PRICE'].", ".$row['QTY'].", ".$row['DIS_per'].", ".$row['DIS_rs'].", ".($row['PRICE']*$row['QTY']).", '".$row['BRAND']."', '".$_SESSION["tmp_no_ord"]."')") or die(mysqli_error());
			...
			while($row = mysqli_fetch_array($sql)){
				...
				$ad="ad".$i;	
             	...
             	$ResponseXML .= "<tr>  
							...
							$sql12 = mysqli_query($GLOBALS['dbinv'],"update tmp_inv_data set ad='1' where id=".$row['id']." and str_code='".$row['str_code']."' and tmp_no= '".$_SESSION["tmp_no_salinv"]."'") or die(mysqli_error());
