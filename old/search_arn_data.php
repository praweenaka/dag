	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
                            <tr>
                              ...
                              <td width=\"424\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">LC No</font></td>
							...
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
								$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_purmas where CANCEL!='1' and  REFNO like  '$letters%' order by id desc") or die(mysqli_error());
							} else if ($_GET["mstatus"]=="lcno"){
								$letters = $_GET['lcno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_purmas where CANCEL!='1' and LCNO like  '$letters%' order by id desc") or die(mysqli_error()) or die(mysqli_error());
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_purmas where CANCEL!='1' and SUP_NAME like  '$letters%' order by id desc") or die(mysqli_error()) or die(mysqli_error());
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_purmas where CANCEL!='1' and SUP_NAME like  '$letters%' order by id desc") or die(mysqli_error()) or die(mysqli_error());
