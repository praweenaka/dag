...
if ($_GET["Command"] == "pass_grn") {
    ...
    if ($row = mysqli_fetch_array($sql)) {
        ...
        while ($row_data = mysqli_fetch_array($sql_data)) {
            ...
            if ($row['VAT'] == "1") {                 
                $PRICE = $row_data['PRICE'] / (1 + ($vatrate / 100));  	
            } else {
            ...
            $totqty = 0;
            $sql_ret_mast = mysqli_query($GLOBALS['dbinv'], "Select * from s_crnma where INVOICENO='" . $inv . "' and CANCELL!='1'") or die(mysqli_error());
            while ($row_ret_mast = mysqli_fetch_array($sql_ret_mast)) {
                $sql_ret = mysqli_query($GLOBALS['dbinv'], "Select sum(QTY) as totqty from s_crntrn where REF_NO='" . $row_ret_mast["REF_NO"] . "' and STK_NO='" . $row_data['STK_NO'] . "' and CANCELL!='1'") or die(mysqli_error());
...
if ($_GET["Command"] == "pass_invno") {
    ...
    if ($row = mysqli_fetch_array($sql)) {
        ...
        if (($VAT_add == "1") or ( $VAT_add == "2") or ( $VAT_add == "3")) {
               $vat_val = $row['AMOUNT'] * $row_invpara["vatrate"] / 100;            
        } else {
            $vat_val = 0;
        }
