...
if ($_GET["Command"] == "search_item") {
    ...
    require_once ("new/connection_sql.php");
    ...
    $ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
        ...
        if ($_GET['itno'] != "") {
            $sql .= " and stk_no like '" . $_GET['itno'] . "%'";
        }
        if ($_GET['itemname'] != "") {
            $sql .= " and DESCRIPT like '" . $_GET['itemname'] . "%'";
        }
        if (isset($_SESSION["brand"]) == true) { 
          if ($_GET["stname"] == "sales_order") {
                if ($_SESSION["brand"]=="PRESA") {
                  if(($_SESSION["tbrsession"]) == "ok"){
                      $sql .= " and (STK_NO='P1111' or STK_NO='P1047' or STK_NO='P1073' or STK_NO='p1076') and BRAND_NAME ='" . $_SESSION["brand"] . "'";
                  } else{
                    $sql .= " and (STK_NO!='P1111' and STK_NO!='P1047' and STK_NO!='P1073' and STK_NO!='p1076') and BRAND_NAME ='" . $_SESSION["brand"] . "'";
                  } 
              }else{
    ...
    foreach ($conn->query($sql) as $row) {
         if ($_SESSION["stname"] == "weekly_order") {
            ...
            $ResponseXML .= "<tr>               
                            ...
                            <td onclick=\"itno_grn('" . $row['STK_NO'] . "', '" . $_GET["stname"] . "');\">" . $row['QTYINHAND'] . "</a></td></tr>";
        } else if ($_GET["stname"] == "quot") {

            $ResponseXML .= "<tr>               
                              <td onclick=\"itno_quot('" . $row['STK_NO'] . "');\">" . $row['STK_NO'] . "</a></td>
                              <td onclick=\"itno_quot('" . $row['STK_NO'] . "');\">" . $row['DESCRIPT'] . "</a></td>
                <td onclick=\"itno_quot('" . $row['STK_NO'] . "');\">" . $row['BRAND_NAME'] . "</a></td>
                <td onclick=\"itno_quot('" . $row['STK_NO'] . "');\">" . $row['QTYINHAND'] . "</a></td></tr>";
        } else if (($_GET["mstatus"] == "itno") or ( $_GET["mstatus"] == "itemname")) {
            $ResponseXML .= "<tr>
                ...
                <td onclick=\"itno('" . $row['STK_NO'] . "', '" . $_GET["stname"] . "');\">" . $row['SELLING'] . "</td>";
...
//echo $res;
    ...
    if ($_GET["mstatus"] == "itno") {
        ...
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
        if ($_GET["brand"] == "All") {
            $sql = mysqli_query($GLOBALS['dbinv'], "SELECT * from s_mas where STK_NO like  '$letters%' order by DESCRIPT limit 50") or die(mysqli_error());
        } else {
            $sql = mysqli_query($GLOBALS['dbinv'], "SELECT * from s_mas where BRAND_NAME='" . $_GET["brand"] . "' and STK_NO like  '$letters%' order by DESCRIPT limit 50") or die(mysqli_error());
        }
        ...
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
        if ($_GET["brand"] == "All") {
            $sql = mysqli_query($GLOBALS['dbinv'], "SELECT * from s_mas where DESCRIPT like  '$letters%' order by DESCRIPT limit 50") or die(mysqli_error());
        } else {
            $sql = mysqli_query($GLOBALS['dbinv'], "SELECT * from s_mas where BRAND_NAME='" . $_GET["brand"] . "' and DESCRIPT like  '$letters%' order by DESCRIPT limit 50") or die(mysqli_error());
        }
    } else {
