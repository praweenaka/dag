		...
		//$ResponseXML .= "<invdetails>";
				...
				if ($_GET["mstatus"]=="invno"){
					$letters = $_GET['invno'];
								...
								//echo $a;
					$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_ginmas where cancel='0' and del_to = 'N' and REF_NO like  '$letters%' order by SDATE desc") or die(mysqli_error());
				} else if ($_GET["mstatus"]=="from"){
					$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
					$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_ginmas where cancel='0' and del_to = 'N' and DEP_F_NAME like  '$letters%' order by SDATE desc") or die(mysqli_error());
				} else if ($_GET["mstatus"]=="to"){
					$letters = $_GET['invdate'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
					$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_ginmas where cancel='0' and del_to = 'N' and DEP_T_NAME like  '$letters%' order by SDATE desc") or die(mysqli_error());
				} else {
					$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
					$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_ginmas where cancel='0' and del_to = 'N' order by SDATE desc") or die(mysqli_error());
				}
	...
	/*		$ResponseXML .= "<sales_table><![CDATA[ <table><tr>
			...
			//echo "Select * from s_mas where STK_NO='".$_GET['itno']."' and BRAND_NAME='".$_GET["brand"]."'";
				...
				if($row = mysqli_fetch_array($sql)){
					...
					$sql1 = mysqli_query($GLOBALS['dbinv'],"Select sum(REC_QTY) as stk from s_purtrn where STK_NO='".$_GET['itno']."' and CANCEL='0' and SDATE>'".$dt."' ") or die(mysqli_error());
				...
				$ResponseXML .= " <itemlist><![CDATA[ <table class=\"table table-bordered\">
				<tr>
				<td width=\"122\">Department</font></td>
				<td width=\"51\">Stock</font></td>
				<td width=\"51\">Pending Order</font></td>

				</tr>";

				$sql2 = mysqli_query($GLOBALS['dbinv'], "Select * from s_stomas");

				while ($row2 = mysqli_fetch_array($sql2)) {

					$sql3 = mysqli_query($GLOBALS['dbinv'], "Select QTYINHAND from s_submas where STO_CODE='" . $row2["CODE"] . "' and STK_NO='" . $_GET["itno"] . "' and qtyinhand >0") or die(mysql_error());
					if ($row3 = mysqli_fetch_array($sql3)) {
						$mord = 0;
 
						$sql4 =  mysqli_query($GLOBALS['dbinv'], "Select sum(qty) as qty from view_cusordmas_trn where department='" . $row2["CODE"] . "' and STK_NO='" . $_GET["itno"] . "' and cancell='0' and invno ='0'") or die(mysql_error());
						if ($row4 = mysqli_fetch_array($sql4)) {
							$mord = $row4['qty'];
						}
 
						$ResponseXML .= "<tr>
						<td width=\"122\" >" . $row2["DESCRIPTION"] . "</font></td>
						<td width=\"51\"  align=right>" . number_format($row3["QTYINHAND"], 0, ".", ",") . "</font></td> 
						<td width=\"51\"  align=right>" . $mord . "</font></td>";
