...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
                            <tr>
                              ...
                              <td width=\"424\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Cheque Date</font></td>
							  <td width=\"424\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Cheque Amount</font></td>
							  <td width=\"424\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">ID</font></td>
							  <td width=\"424\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Ref No</font></td>
                           
   							</tr>";
                           
						   $letters = $_GET['cusno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select cheque_no, che_amount, che_date, bank, refno, CUS_NAME, ID, ret_refno  from s_invcheq where cheque_no like '$letters%' ORDER BY che_date desc limit 50";
							...
							/*if ($_GET["mstatus"]=="cusno"){
						   		$letters = $_GET['cusno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select c_code,c_name from lcodes where c_code like '$letters%' ORDER BY c_code limit 50";
								$result=mysqli_query($GLOBALS['dbinv'],$sql, $dbacc);
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select c_code,c_name from lcodes where c_name like '$letters%' ORDER BY c_code limit 50";
								$result=mysqli_query($GLOBALS['dbinv'],$sql, $dbacc);
							...
							$ResponseXML .= "<tr>               
							  ...
							  <td onclick=\"chqno('$cuscode', '$stname');\">".$row['ID']."</a></td>";
							  if (($row['ret_refno']!='1') and ($row['ret_refno']!='0')){	   
							  	$ResponseXML .= "<td onclick=\"chqno('$cuscode', '$stname');\">".$row['ret_refno']."</td>";	
							  } else {
							  	$ResponseXML .= "<td onclick=\"chqno('$cuscode', '$stname');\"></td>";
							  }	                             
                            $ResponseXML.= "</tr>";
							}
					echo $ResponseXML;
...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
                            <tr>
                              <td width=\"121\"  background=\"images/headingbg.gif\" ><font color=\"#FFFFFF\">Account No</font></td>
                              <td width=\"424\"  background=\"images/headingbg.gif\"><font color=\"#FFFFFF\">Account</font></td>
...
if ($_GET["Command"]=="pass_chq_return"){
	...
	$sql="select c_code, c_name from lcodes where c_code='".$_GET["ledgno"]."'";
	$result=mysqli_query($GLOBALS['dbacc'],$sql);
...
if ($_GET["Command"]=="pass_ledger_sel"){
	...
	if($row = mysqli_fetch_array($result)){							
		...
		$ResponseXML .= "<c_add2><![CDATA[".$row['c_add2']."]]></c_add2>";
		$ResponseXML .= "<c_opbal><![CDATA[".$row['c_opbal']."]]></c_opbal>";
		$ResponseXML .= "<c_type><![CDATA[".$row['c_type']."]]></c_type>";
	}
...
if ($_GET["Command"]=="tmp_crelimit"){
	...
	if($row = mysqli_fetch_array($sql)){
		...
		$sql_data = mysqli_query($GLOBALS['dbinv'],"insert into tmpCrLmt (sdate, stime, username, tmpLmt, Class, Rep, cusCode, crLmt, cat) values ('".date("Y-m-d")."', '".time("H:m:s")."', 'Test_Admin', ".$_GET["txt_templimit"].", '".trim($_GET["cmbbrand1"])."', '".$Com_rep1."', '".trim($_GET["txt_cuscode"])."', ".$crelim.", '".$cat."' )") or die(mysqli_error());
...
$sql = mysqli_query($GLOBALS['dbinv'],"Select * from br_trn where cus_code='".trim($_GET["txt_cuscode"])."' and Rep='".$Com_rep1."' and  brand='".$_GET["cmbbrand1"]."'") or die(mysqli_error());
	...
	//	echo "update br_trn set tmpLmt= ".$_GET["txt_templimit"]." where cus_code='".trim($_GET["txt_cuscode"])."' and Rep='".$Com_rep1."' and brand='".$_GET["cmbbrand1"]."'";
		$sql_data = mysqli_query($GLOBALS['dbinv'],"update br_trn set tmpLmt= ".$_GET["txt_templimit"]." where cus_code='".trim($_GET["txt_cuscode"])."' and Rep='".$Com_rep1."' and brand='".$_GET["cmbbrand1"]."'") or die(mysqli_error());
	} else {
	
		$sql_data = mysqli_query($GLOBALS['dbinv'],"insert into br_trn (cus_code, Rep, credit_lim, brand, tmpLmt) values ('".trim($_GET["txt_cuscode"])."', '".$Com_rep1."', '0', '".$_GET["cmbbrand1"]."', '".$_GET["txt_templimit"]."')") or die(mysqli_error());
	}
...
if ($_GET["Command"]=="pass_cus_cash_rec")
{
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			if ($_GET['refno']==""){
				$sql = mysqli_query($GLOBALS['dbinv'],"Select C_CODE, REF_NO, deli_date, GRAND_TOT, TOTPAY, SDATE from s_salma where C_CODE='".$_GET['custno']."' and GRAND_TOT > TOTPAY and CANCELL='0' ORDER BY SDATE") or die(mysqli_error());
			} else {
			
				$sql = mysqli_query($GLOBALS['dbinv'],"Select C_CODE, REF_NO, deli_date, GRAND_TOT, TOTPAY, SDATE from s_salma where C_CODE='".$_GET['custno']."' and GRAND_TOT > TOTPAY and CANCELL='0' and SAL_EX='".$_GET['refno']."'  ORDER BY SDATE") or die(mysqli_error());
			}	
				while($row = mysqli_fetch_array($sql)){
