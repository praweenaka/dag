...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="cusno"){
						   		$letters = $_GET['cusno'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select c_code,c_name from lcodes where cat='B' and c_code like '$letters%' ORDER BY c_code limit 50";
								$result=mysqli_query($GLOBALS['dbinv'],$sql, $dbacc);
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql="select c_code,c_name from lcodes where cat='B' and c_name like '$letters%' ORDER BY c_code limit 50";
								$result=mysqli_query($GLOBALS['dbinv'],$sql, $dbacc);
...
if ($_GET["Command"]=="pass_cash_rec"){
	...
	$sql="select c_code, c_name from lcodes where c_code='".$_GET["ledgno"]."' and cat='B'";
	$result=mysqli_query($GLOBALS['dbinv'],$sql, $dbacc);
