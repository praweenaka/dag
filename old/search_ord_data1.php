						...
						if ($_GET["Option1"]=="true"){
							if ($_GET["mstatus"]=="invno"){
								...
								if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where REF_NO like  '$letters%' and  INVNO='0' and CANCELL='0' limit 50") or die(mysqli_error());
								}	
								...
								if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and CANCELL='0' limit 50") or die(mysqli_error()) or die(mysqli_error());
								}
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
						...
						if ($_GET["Option2"]=="true"){
							if ($_GET["mstatus"]=="invno"){
								...
								if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where approveby!='0' and REF_NO like  '$letters%' and  INVNO='0' and CANCELL='0' limit 50") or die(mysqli_error());
								}	
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where approveby!='0' and CUS_NAME like  '$letters%' and  INVNO='0' and CANCELL='0' limit 50") or die(mysqli_error()) or die(mysqli_error());
								}
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
...
FROM         brand_mas RIGHT OUTER JOIN
				...
				if ($NewRefNo==$salesrep){
					$sqlinvcheq = mysqli_query($GLOBALS['dbinv'],"SELECT * FROM s_invcheq WHERE che_date>'".date("Y-m-d")."' AND cus_code='".trim($cuscode)."' and trn_type='REC' and (sal_ex='".$OldRefno."' or sal_ex='".trim($salesrep)."'") or die(mysqli_error());
				} else {	
					$sqlinvcheq = mysqli_query($GLOBALS['dbinv'],"SELECT * FROM s_invcheq WHERE che_date>'".date("Y-m-d")."' AND cus_code='".trim($cuscode)."' and trn_type='REC' and sal_ex='".trim($salesrep)."'") or die(mysqli_error());
				}
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
		...
		if ($_SESSION['company']=="THT"){	
			...
			if ($_SESSION['dev']=="1"){
				...
				$invno=trim("CRI/ ").substr($tmpinvno, $lenth-7)."/".$SAL_EX;
				$_SESSION["invno"]=$invno;
			...
			$sql = mysqli_query($GLOBALS['dbinv'],"Select SPINV, CAS_INV_NO_m, CAS_INV_NO from invpara") or die(mysqli_error());
			...
			if ($_SESSION['dev']=="1"){
				...
				$invno=trim("BEN/CR/ ").substr($tmpinvno, $lenth-7)."/5";
				$_SESSION["invno"]=$invno;
				...
				$invno=trim("BEN/CR/ ").substr($tmpinvno, $lenth-7)."/".$SAL_EX;
				$_SESSION["invno"]=$invno;
				$txtdono=$row["CAS_INV_NO"]+1;
			...
			$sql = mysqli_query($GLOBALS['dbinv'],"Select * from tmp_inv_data where tmp_no='".$_SESSION["tmp_no_salinv"]."'") or die(mysqli_error());
			while($row = mysqli_fetch_array($sql)){
				...
				$ad="ad".$i;	
             	...
             	$ResponseXML .= "<tr>  
							 ...
							 <td align=right ><input type=\"text\"  class=\"text_purchase3\" name=\"".$actual_selling."\" id=\"".$actual_selling."\" size=\"15\"  value=\"".number_format($rowmas['SELLING'], 2, ".", ",")."\"  /></td>
...
FROM         brand_mas RIGHT OUTER JOIN
 ...
 /*       $pend_ret_set = 0;
      	$sql = mysqli_query($GLOBALS['dbinv'],"SELECT sum(che_amount) as  che_amount FROM s_invcheq WHERE CHE_DATE >'".date("Y-m-d")."' AND CUS_CODE='".trim(txt_cuscode)."' and trn_type='RET' ") or die(mysqli_error());
		if ($row = mysqli_fetch_array($sql)){	
