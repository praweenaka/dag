...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="cusno"){
						   		$letters = $_GET['cusno'];
								$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from vendor where CODE like  '$letters%' and CAT<>'X' order by CODE") or die(mysqli_error());
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from vendor where NAME like  '$letters%' and CAT<>'X' order by CODE") or die(mysqli_error()) or die(mysqli_error());
							} else {
...
FROM         brand_mas RIGHT OUTER JOIN
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			$ResponseXML .= "<salesdetails>";
				...
				$sql = mysqli_query($GLOBALS['dbinv'],"Select * from vendor where CODE='".$suppno."'") or die(mysqli_error());
				if($row = mysqli_fetch_array($sql)){
