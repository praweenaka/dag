...
if ($_GET["Command"]=="tmp_crelimit"){
	...
	if($row = mysqli_fetch_array($sql)){
		...
		$sql_data = mysqli_query($GLOBALS['dbinv'],"insert into tmpCrLmt (sdate, stime, username, tmpLmt, Class, Rep, cusCode, crLmt, cat) values ('".date("d-m-Y")."', '".time("H:m:s")."', 'Test_Admin', ".$_GET["txt_templimit"].", '".trim($_GET["cmbbrand1"])."', '".$Com_rep1."', '".trim($_GET["txt_cuscode"])."', ".$crelim.", '".$cat."' )") or die(mysqli_error());
...
$sql = mysqli_query($GLOBALS['dbinv'],"Select * from br_trn where cus_code='".trim($_GET["txt_cuscode"])."' and Rep='".$Com_rep1."' and  brand='".$_GET["cmbbrand1"]."'") or die(mysqli_error());
	if($row = mysqli_fetch_array($sql)){
		...
		$sql_data = mysqli_query($GLOBALS['dbinv'],"update br_trn set tmpLmt= ".$_GET["txt_tmeplimit"]." where cus_code='".trim($_GET["txt_cuscode"])."' and Rep='".$Com_rep1."' and brand='".$_GET["cmbbrand1"]."'") or die(mysqli_error());
	} else {
...
FROM         brand_mas RIGHT OUTER JOIN
	...
	if ($row = mysqli_fetch_array($sql)){	
		...
		$sqlchq = mysqli_query($GLOBALS['dbinv'],"SELECT che_amount FROM S_INVCHeQ WHERE che_date>'".date("d-m-Y")."' AND cus_code='".$row["CODE"]."'") or die(mysqli_error());
		...
		if ($mdays > 60){
			...
			if (is_null($row["Over_DUE_IG_Date"])==false){
				if ($row["Over_DUE_IG_Date"]==date("d-m-Y")){
					...
					$sqltmpCrLmt = mysqli_query($GLOBALS['dbinv'],"insert into tmpCrLmt (sdate, stime, tmpLmt, Class, Rep,cuscode, crLmt,  FLAG) values ('".date("d-m-Y")."','".date("h:i:s")."' , ".$mdays." ,'NB','NR','".trim($cuscode)."','0', 'O60')") or die(mysqli_error());
	...
	if ($rows_cheq = mysqli_fetch_array($sqls_cheq)){
		...
		if (is_null($row["Over_DUE_IG_Date"])==false){
			if ($row["Over_DUE_IG_Date"]==date("d-m-Y")){
				...
				$sqltmpCrLmt = mysqli_query($GLOBALS['dbinv'],"insert into tmpCrLmt (sdate, stime, tmpLmt, Class, Rep,cuscode, crLmt, FLAG) values ('".date("d-m-Y")."', '".date("h:i:s")."', '0' ,'NB','NR','".trim($cuscode)."','0', 'RTN'") or die(mysqli_error());
			} else {
...
//echo $OutInvAmt;
$sqlinvcheq = mysqli_query($GLOBALS['dbinv'],"SELECT * FROM s_invcheq WHERE che_date>'".date("d-m-Y")."' AND cus_code='".trim($cuscode)."' and trn_type='REC' and sal_ex='".trim($_GET["salesrep"])."'") or die(mysqli_error());
