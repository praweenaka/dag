...
<tr>
      ...
      <td width="603"  background="images/headingbg.gif" ><input type="text" size="70" name="customername" id="customername" value="" class="txtbox" onkeyup="<?php echo "update_cust_list('$stname')"; ?>"/></td>
                ...
                <div id="filt_table" class="CSSTableGenerator">  <table width="735" border="0" class=\"form-matrix-table\">
							...
							$sql="select refno , bdate, amount, name from recmas  ORDER BY bdate desc limit 50";
							...
							echo "<tr>               
                              <td onclick=\"sel_rec_ent('$cuscode', '$stname');\">".$row['refno']."</a></td>
                              <td onclick=\"sel_rec_ent('$cuscode', '$stname');\">".date("Y-m-d", strtotime($row['bdate']))."</a></td>
                         		<td onclick=\"sel_rec_ent('$cuscode', '$stname');\">".$row['name']."</a></td>
								<td onclick=\"sel_rec_ent('$cuscode', '$stname');\">".$row['amount']."</a></td>
