...
while ($row_smas = mysqli_fetch_array($result_smas)) {
    ...
    $sql = "insert into s_trn (SDATE, STK_NO, REFNO, QTY, LEDINDI, DEPARTMENT) values ('" . $_GET["dtfrom"] . "', '" . $row_smas["STK_NO"] . "' , '" . $entREFNO . "', " . number_format($stkqty, 0, ".", "") . ", 'TRN', '01')";
    $result = mysqli_query($GLOBALS['dbinv'], $sql);

    $sql = "update s_submas set QTYINHAND=" . number_format($stkqty, 0, ".", "") . " where  STK_NO='" . $row_smas["STK_NO"] . "' and STO_CODE='01'";
    $result = mysqli_query($GLOBALS['dbinv'], $sql);
