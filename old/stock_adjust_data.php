...
if ($_GET["Command"] == "save_item") {
    ...
    if ($row = mysqli_fetch_array($result)) {
        ...
        while ($row2 = mysqli_fetch_array($result2)) {
            ...
            $sql5 = "Insert into s_trn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT) values('" . $row2["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($_GET["invno"]) . "', " . $cur_qty . ", '" . $_GET["ttype"] . "', '" . trim($_GET["dep"]) . "')";
        ...
        $sqlen = "insert into entry_log(refno, username, docname, trnType, stime, sdate) values ('" . $_GET['invno'] . "', '" . $_SESSION["CURRENT_USER"] . "', 'Stock Adjest', 'Save', '" . date("Y-m-d H:i:s") . "', '" . date("Y-m-d") . "')";
        $resulten = mysqli_query($GLOBALS['dbinv'], $sqlen);
...
if ($_GET['Command'] == "cancel_inv") {
    ...
    $sql = "select * from s_trn where REFNO='" . $_GET["invno"] . "'";
    $result = mysqli_query($GLOBALS['dbinv'], $sql);
    while ($row = mysqli_fetch_array($result)) {
    ...
    $sql1 = "update inreq_mas set cancell='1' where refno='" . $_GET["invno"] . "'";
    $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);

    $sql1 = "update inreq_trn set cancell='1' where refno='" . $_GET["invno"] . "'";
    $result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
    ...
    $sqlen = "insert into entry_log(refno, username, docname, trnType, stime, sdate) values ('" . $_GET['invno'] . "', '" . $_SESSION["CURRENT_USER"] . "', 'Stock Adjest', 'Cancel', '" . date("Y-m-d H:i:s") . "', '" . date("Y-m-d") . "')";
    $resulten = mysqli_query($GLOBALS['dbinv'], $sqlen);
