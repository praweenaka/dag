...
{   
		...
		//alert(inv_status);
			...
			url=url+"&book_no="+document.getElementById('book_no').value;
			...
			myString=document.getElementById('supplier_name').value;
			myString = myString.replace(/&/g,"~");
			url=url+"&supplier_name="+myString;
			
			myString=document.getElementById('textarea').value;
			myString = myString.replace(/&/g,"~");
			url=url+"&textarea="+myString;
...
function result_pass_arnno()
{
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("book_no");	
		opener.document.form1.book_no.value = XMLAddress1[0].childNodes[0].nodeValue;
