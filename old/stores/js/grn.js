	  ...
	  if (valqty>=totret){
	   ...
	   valprice=parseFloat(valprice.replace(/,/gi, ""));
			...
			if (document.getElementById('vatgroup_1').checked==false){
				document.getElementById('tax').value=(before_vat*12)/100;
				//alert(before_vat);
...
{   
		...
		//	document.getElementById('discount3').disabled=true;
			...
			xmlHttp.onreadystatechange=showarmyresultdel2;
			xmlHttp.open("GET",url,true);
			xmlHttp.send(null);
...
function pass_grnno_result()
{
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
	{
		...
		XMLAddress1 = xmlHttp.responseXML.getElementsByTagName("serial_table");	
...
{   
			...
			document.getElementById('itemd').value="";
			document.getElementById('qty').value="";
			document.getElementById('txtstoRef').value="";
...
function assign_invno(){
	...
	document.getElementById('txtstoRef').focus();
