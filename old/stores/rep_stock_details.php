        ...
        <td width="106"><select name="department" id="department" onkeypress="keyset('brand',event);" class="text_purchase3">
            <?php
				echo "<option value='All'>All</option>";
                  if (!is_numeric($_SESSION["CURRENT_DEP"]))  {							 						
                    ...
                    $sql="select * from s_stomas_stores  where CODE = '" . $_SESSION["CURRENT_DEP"] ."' order by CODE";
					$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
                    while($row = mysqli_fetch_array($result)){
                    echo "<option value='".$row["CODE"]."'>".$row["CODE"]." ".$row["DESCRIPTION"]."</option>";
                    }
  ...
  <tr>
    ...
    <td colspan="2"><select name="cmb_t" id="cmb_t" class="text_purchase3">
						...
						<option value='All' selected="selected">All</option>
							<?php
							$sql = "select costcenter from brand_mas group by costcenter";
							$result = mysqli_query($GLOBALS['dbinv'], $sql);
							while ($row = mysqli_fetch_array($result)) {
