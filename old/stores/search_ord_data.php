		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
						...
						if ($_GET["Option1"]=="true"){
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
								/*if ($_GET["stname"]=="ord"){
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where REF_NO like  '$letters%' limit 50") or die(mysqli_error());
								} else {*/
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where REF_NO like  '$letters%' and  INVNO='0' and CANCELL='0'  order by SDATE desc limit 50") or die(mysqli_error());
								//}	
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								/*if ($_GET["stname"]=="ord"){
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where CUS_NAME like  '$letters%' limit 50 ") or die(mysqli_error()) or die(mysqli_error());
								} else {*/
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and CANCELL='0'  order by SDATE desc limit 50") or die(mysqli_error()) or die(mysqli_error());
								//}
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
						...
						if ($_GET["Option2"]=="true"){
							if ($_GET["mstatus"]=="invno"){
								...
								/*if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where REF_NO like  '$letters%' and  INVNO='0' and CANCELL='0'  and Result='OK' order by SDATE desc limit 50") or die(mysqli_error());
								//}	
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
							/*	if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and CANCELL='0' and Result='OK'  order by SDATE desc limit 50") or die(mysqli_error()) or die(mysqli_error());
								//}
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
							/*	if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and CANCELL='0' and Result='OK'  order by SDATE desc limit 50") or die(mysqli_error()) or die(mysqli_error());
									//echo "SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and CANCELL='0' and Result='OK'  order by SDATE desc limit 50";
								//}
							}
						}	
						...
						if ($_GET["Option4"]=="true"){
							if ($_GET["mstatus"]=="invno"){
								...
								/*if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where REF_NO like  '$letters%' and  INVNO='0' and Forward='WD' and CANCELL='0' and Result='P' order by SDATE desc limit 50") or die(mysqli_error());
								//}	
							} else if ($_GET["mstatus"]=="customername"){
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
								/*if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and Forward='WD' and CANCELL='0' and Result='P' order by SDATE desc limit 50") or die(mysqli_error()) or die(mysqli_error());
								//}
							} else {
								$letters = $_GET['customername'];
								//$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
							/*	if ($_GET["stname"]=="ord"){
									...
									$sql = mysqli_query($GLOBALS['dbinv'],"SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and Forward='WD' and CANCELL='0' and Result='P' order by SDATE desc limit 50") or die(mysqli_error()) or die(mysqli_error());
									//echo "SELECT * from s_cusordmas where CUS_NAME like  '$letters%' and  INVNO='0' and Forward='WD' and CANCELL='0' and Result='P' order by SDATE desc limit 50";
								//}
							}
						}	
...
if ($_GET["Command"]=="pass_invno_cus_ord"){
	...
	if($row_rssalma = mysqli_fetch_array($sql)){
		...
		if($row_cus = mysqli_fetch_array($sql_cus)){
			$ResponseXML .= "<lblvatno><![CDATA[".$row_rssalma["vatno"]."]]></lblvatno>";
			$ResponseXML .= "<txt_cusadd><![CDATA[".$row_rssalma["ADD1"]." ".$row_rssalma["ADD2"]."]]></txt_cusadd>";
	...
	// Call setcus
        ...
        while($row_rsinv = mysqli_fetch_array($sql_rsinv)){
			...
			('".$_GET["invno"]."', '".$row_rsinv["STK_NO"]."', '".$row_rsinv["DESCRIPT"]."', ".$PRICE.", ".$row_rsinv["QTY"].", ".$row_rsinv["DIS_per"].", ".$row_rsinv["DIS_rs"].", ".($row_rsinv["QTY"]*$PRICE).", '".$row_rsinv["STK_NO"]."', '".$row_rsinv["STK_NO"]."')") or die(mysqli_error());	
			...
			$ResponseXML .= "<tr>
							 ...
							 <td >".number_format((($row_rsinv["QTY"]*$PRICE)-$row_rsinv["DIS_per"]), 2, ".", ",")."</a></td>
...
FROM         brand_mas RIGHT OUTER JOIN
				...
				if ($NewRefNo==$salesrep){
					$sqlinvcheq = mysqli_query($GLOBALS['dbinv'],"SELECT * FROM s_invcheq WHERE che_date>'".$_GET["invdate"]."' AND cus_code='".trim($cuscode)."' and trn_type='REC' and (sal_ex='".$OldRefno."' or sal_ex='".trim($salesrep)."'") or die(mysqli_error());
				} else {	
...
header('Content-Type: text/xml'); 
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			$ResponseXML .= "<stname><![CDATA[".$_GET["stname"]."]]></stname>";
				...
				$vatmethod="";	
					//echo "Select * from s_cusordmas where REF_NO='".$inv."'";
				$sql = mysqli_query($GLOBALS['dbinv'],"Select * from s_cusordmas where REF_NO='".$inv."'") or die(mysqli_error());
		...
		if ($_SESSION['company']=="THT"){	
			...
			$sql = mysqli_query($GLOBALS['dbinv'],"Select SPINV, SPINV1, CAS_INV_NO_m, CAS_INV_NO from invpara") or die(mysqli_error());
			...
			//('".$_GET['invno']."', '".$_GET['itemcode']."', '".$_GET['item']."', ".$rate.", ".$qty.", ".$_GET["discountper"].", ".$discount.", ".$subtotal.", '".$_GET["brand"]."', '".$_SESSION["tmp_no_ord"]."', '".$_GET["str_partno"]."') ";
					...
					if ($vatmethod!="1"){
						$SELLING=$rowit['SELLING'];
					} else {
						$SELLING=$rowit['SELLING']/($vatrate+1);
					}
			...
			while($row = mysqli_fetch_array($sql)){
				...
				$ad="ad".$i;	
             	...
             	$ResponseXML .= "<tr>  
							 ...
							 <td align=right onClick=\"disp_qty('".$row['str_code']."');\"><input type=\"text\"  class=\"text_purchase3\" name=\"".$rate."\" id=\"".$rate."\" size=\"15\"  value=\"".number_format($row['cur_rate'], 2, ".", ",")."\"  disable  onkeypress=\"keyset('credper',event);\" /></td>
							 <td align=right ><input type=\"text\"  class=\"text_purchase3\" name=\"".$actual_selling."\" id=\"".$actual_selling."\" size=\"15\"  value=\"".number_format($rowmas['SELLING'], 2, ".", ",")."\" onblur=\"calc1_table('".$i."');\" /></td>
			...
			//	echo "Select sum(cur_subtot) as tot_sub, sum(cur_discount) as tot_dis from tmp_inv_data where tmp_no='".$_SESSION["tmp_no_salinv"]."'";
				$sql = mysqli_query($GLOBALS['dbinv'],"Select sum(cur_subtot) as tot_sub, sum(cur_discount) as tot_dis from tmp_inv_data where tmp_no='".$_SESSION["tmp_no_salinv"]."'") or die(mysqli_error());	
...
//echo $OutInvAmt;
$sqlinvcheq = mysqli_query($GLOBALS['dbinv'],"SELECT * FROM s_invcheq WHERE che_date>'".$_GET["invdate"]."' AND cus_code='".trim($cuscode)."' and trn_type='REC' and sal_ex='".trim($salrep)."'") or die(mysqli_error());
	while($rowinvcheq = mysqli_fetch_array($sqlinvcheq)){
