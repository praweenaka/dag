...
////////////////////////////////////////////// Database Connector /////////////////////////////////////////////////////////////
	...
	/*		$sql="Select * from tmp_army_no where edu= '".$_GET['edu']."'";
		...
		/*	$sql="Select CAS_INV_NO_m from invpara";
			...
			$sql="Select adj from invpara_stores";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			...
			$sql="delete from tmp_stock_adjust_data_stores where str_invno='".$invno."'";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
	...
	if($_GET["Command"]=="add_tmp")
		{
			...
			$sql="Insert into tmp_stock_adjust_data_stores (str_invno, str_code, str_description, partno, cur_qty)values 
			...
			$sql="Select * from tmp_stock_adjust_data_stores where str_invno='".$_GET['invno']."'";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 	
...
if($_GET["Command"]=="save_item")
{
			...
			$sql="select * from inreq_mas_stores where refno='".$_GET["invno"]."' ";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			if($row = mysqli_fetch_array($result)){
				echo "Stock Adjust Number alredy exists";
			} else {
								
				$sql1="insert into inreq_mas_stores(refno, sdate, dep, trn_type, remark, CANCELL) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$_GET["dep"]."', '".$_GET["ttype"]."', '".$_GET["spinst"]."', '0')";
				//echo $sql1;
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
			
				
				$sql2="select * from tmp_stock_adjust_data_stores where str_invno='".$_GET["invno"]."' ";
				//echo $sql2;
				...
				while($row2 = mysqli_fetch_array($result2)){
					...
					$sql1="insert into inreq_trn_stores(refno, sdate, stk_no, trn_type, qty, CANCELL) values ('".$_GET["invno"]."', '".$_GET["invdate"]."', '".$row2["str_code"]."', '".$_GET["ttype"]."', '".$cur_qty."', '0')";
					//echo $sql1;
					$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
						
					if ($_GET["ttype"]=="IIN"){					
						$sql1="update s_mas set QTYINHAND_STORES=QTYINHAND_STORES+".$cur_qty." where STK_NO='".$row2["str_code"]."' ";
						//echo $sql1;
						$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
					...
					if ($_GET["ttype"]=="IIN"){					
						$sql1="update s_submas_stores set QTYINHAND=QTYINHAND+".$cur_qty." where STK_NO='".$row2["str_code"]."' and STO_CODE='".$_GET["dep"]."'";
						//echo $sql1;
						$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
					} else if ($_GET["ttype"]=="IOU"){			
						$sql1="update s_submas_stores set QTYINHAND=QTYINHAND-".$cur_qty." where STK_NO='".$row2["str_code"]."' and STO_CODE='".$_GET["dep"]."'";
					...
					$sql5="Insert into s_trn_stores (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT) values('".$row2["str_code"]."', '".$_GET["invdate"]."', '" . trim($_GET["invno"]) . "', ".$cur_qty.", '" . $_GET["ttype"] . "', '" . trim($_GET["dep"]) . "')";
  ...
  //  & " values('" & Trim(myGridCMI.TextMatrix(i, 1)) & "','" & dtdate & "','" & Trim(lblRefNo) & "'," & Val(myGridCMI.TextMatrix(i, 4)) & ",'" & typ & "','" & Trim(Left(cmbdep, 5)) & "')"	
				...
				$sql1="update invpara_stores set adj=adj+1";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
				echo "Saved";
			}
...
if ($_GET['Command'] == "cancel_inv") {
	...
	$sql = "select * from s_trn_stores where REFNO='" . $_GET["invno"] . "'";
	 
	$result = mysqli_query($GLOBALS['dbinv'], $sql);
	while ($row = mysqli_fetch_array($result)) {

		if ($row["LEDINDI"] == "IIN") {
			$sql1 = "update s_mas set QTYINHAND_STORES=QTYINHAND_STORES-" . $row['QTY'] . " where STK_NO='" . $row["STK_NO"] . "' ";
			$result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
	...
	$sql1 = "update inreq_mas_stores set cancell='1' where refno='" . $_GET["invno"] . "'";
	$result1 = mysqli_query($GLOBALS['dbinv'], $sql1);

	$sql1 = "update inreq_trn_stores set cancell='1' where refno='" . $_GET["invno"] . "'";
	$result1 = mysqli_query($GLOBALS['dbinv'], $sql1);
