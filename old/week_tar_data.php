...
include_once("connectioni.php");
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			$sql=mysqli_query($GLOBALS['dbinv'],"Select * from week_tragets where refno='".$_GET['refno']."'")or die(mysqli_error());
			if($row = mysqli_fetch_array($sql)){		
				
				$sql_date=mysqli_query($GLOBALS['dbinv'],"Select min(Tardate) as min_Tardate, max(Tardate) as max_Tardate from week_tragets where refno='".$_GET['refno']."'") or die(mysqli_error());
				$row_date = mysqli_fetch_array($sql_date);		
					
				
				$ResponseXML .= "<refno><![CDATA[".$row['refno']."]]></refno>";
				$ResponseXML .= "<dtfrom><![CDATA[".$row['dtfrom']."]]></dtfrom>";
				$ResponseXML .= "<dtfr><![CDATA[".$row_date['min_Tardate']."]]></dtfr>";
				$ResponseXML .= "<dtto><![CDATA[".$row_date['max_Tardate']."]]></dtto>";
			...
			$sql_ven=mysqli_query($GLOBALS['dbinv'],"delete from tmp_weekly_tar where ref_no='".$row['refno']."' ")or die(mysqli_error());
				
			$sql=mysqli_query($GLOBALS['dbinv'],"Select * from week_tragets where refno='".$_GET['refno']."'")or die(mysqli_error());
			while($row = mysqli_fetch_array($sql)){		
				
				$sql_ven=mysqli_query($GLOBALS['dbinv'],"Select * from vendor where CODE='".$row['cus_code']."'")or die(mysqli_error());
				$row_ven = mysqli_fetch_array($sql_ven);	
				
				$sql_ven=mysqli_query($GLOBALS['dbinv'],"Insert into tmp_weekly_tar (sal_ref, ref_no, tar_date, cus_code, cus_name, target, sale, remark)values 
			('".$row['rep']."', '".$row['refno']."', '".$row['Tardate']."', '".$row['cus_code']."', '".$row_ven['NAME']."', ".$row["Target"].", 0, '".$row['remark']."')")or die(mysqli_error());
				
			 	$ResponseXML .= "<tr>
                        	 <td>".$row['Tardate']."</td>
							 <td>".$row['cus_code']."</td>";
							 
							$sql1=mysqli_query($GLOBALS['dbinv'],"Select * from vendor where CODE='".$row['cus_code']."'")or die(mysqli_error());
							$row1 = mysqli_fetch_array($sql1);		
							 ...
							 <td>".$row['remark']."</td>
							 <td><img src=\"images/delete_01.png\" width=\"20\" height=\"20\" id=".$row['str_code']."  name=".$row['str_code']." onClick=\"del_item('".$row['Tardate']."', '".$row['cus_code']."');\"></td></tr>";
